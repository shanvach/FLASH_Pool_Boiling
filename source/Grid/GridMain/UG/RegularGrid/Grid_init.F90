...
#include "constants.h"
  ...
  character(len=MAX_STRING_LENGTH) :: xl_newbcString,xr_newbcString
  ...
  !get the grid stretching conditions stored as strings in the flash.par file
  call RuntimeParameters_get("x_stretch_type", x_strString)
  call RuntimeParameters_get("y_stretch_type", y_strString)
  call RuntimeParameters_get("z_stretch_type", z_strString)
  
  !map the string boundary conditions to integer constants defined in constants.h
  call RuntimeParameters_mapStrToInt(x_strString,gr_iStrType)
  call RuntimeParameters_mapStrToInt(y_strString,gr_jStrType)
  call RuntimeParameters_mapStrToInt(z_strString,gr_kStrType)
