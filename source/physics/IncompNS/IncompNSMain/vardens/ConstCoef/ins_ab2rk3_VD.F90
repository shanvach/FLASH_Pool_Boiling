...
#include "constants.h"
  ...
  use IncompNS_data, ONLY : ins_isgs, ins_invRe, ins_intschm, ins_prescoeff, ins_meshMe,&
                            ...
                            ins_rhoa, AB2_SCHM, RK3_SCHM, ins_tlevel, &
...
!#endif
  ...
  do lb = 1,blockCount
     ...
     call ins_predictor_VD(facexData(VELC_FACE_VAR,:,:,:),&
                        ...
                        solnData(PRES_VAR,:,:,:),      &
                        dt,dx(:,LEFT_EDGE,blockID),dy(:,LEFT_EDGE,blockID),&
                        dz(:,LEFT_EDGE,blockID), &
            blkLimits(LOW,IAXIS),blkLimits(HIGH,IAXIS),&
            blkLimits(LOW,JAXIS),blkLimits(HIGH,JAXIS),&
            blkLimits(LOW,KAXIS),blkLimits(HIGH,KAXIS),&
...
!***********************************************************************************************
  ...
  do lb = 1,blockCount
     ...
     call ins_correctorConstCoef(facexData(VELC_FACE_VAR,:,:,:),&
                           faceyData(VELC_FACE_VAR,:,:,:),&
                           facezData(VELC_FACE_VAR,:,:,:),&
                           ...
                           solnData(PFUN_VAR,:,:,:),&
                           solnData(DFUN_VAR,:,:,:),ins_alfa,blockID)
