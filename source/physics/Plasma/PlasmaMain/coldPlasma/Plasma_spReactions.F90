subroutine Plasma_spReactions(RSP0, RSP1, RSP2, RSP3, RSP4, RSP5, RSP6, RSP7,&
   ...
   do j=jy1,jy2
     do i=ix1,ix2
        ...
        RSP3(i,j,1)  = (1e-6)*(7e-10)
        RSP4(i,j,1)  = (1e-6)*(1e-10)*((1.5*pls_KtoeV*T_e(i,j,1))**1.9)*&
                       EXP(-14.6/(1.5*pls_KtoeV*T_e(i,j,1)))
        RSP5(i,j,1)  = (1e-6)*(1.66*1e-6)/((T_h(i,j,1))**0.7)
        RSP6(i,j,1)  = (1e-6)*(1e-12)
        RSP7(i,j,1)  = (1e-6)*(2.03e-8)*((1.5*pls_KtoeV*T_e(i,j,1))**(-0.1))*&
                       EXP(-8.47/(1.5*pls_KtoeV*T_e(i,j,1)))
        RSP8(i,j,1)  = (1e-6)*(2.63e-10)*((1.5*pls_KtoeV*T_e(i,j,1))**(-0.495))*&
                       EXP(-5.65/(1.5*pls_KtoeV*T_e(i,j,1)))
        RSP9(i,j,1)  = (1e-6)*(9.54e-6)*((1.5*pls_KtoeV*T_e(i,j,1))**(-1.05))*&
                       EXP(-55.6/(1.5*pls_KtoeV*T_e(i,j,1)))
        RSP10(i,j,1) = (1e-6)*(1.68e-5)/(T_h(i,j,1)**0.7)
        RSP11(i,j,1) = (1e-6)*(1.4e-10)
        RSP12(i,j,1) = (1e-6)*(1e-7)
        RSP13(i,j,1) = (1e-6)*(2e-7)*((300.0/(T_h(i,j,1)))**0.5)
        RSP14(i,j,1) = (1e-6)*(2.6e-10)
        RSP15(i,j,1) = (1e-6)*(1.05e-12)*((pls_KtoeV*T_h(i,j,1))**0.5)
        RSP16(i,j,1) = (1e-6)*(4.5e-12)*&
                       EXP(-3320.0/T_h(i,j,1))
