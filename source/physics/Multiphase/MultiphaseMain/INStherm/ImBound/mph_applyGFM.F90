subroutine mph_applyGFM(blockCount, blockList,timeEndAdv,dt,dtOld,sweepOrder)
...
#include "ImBound.h"
  ...
  real :: th, thxp, thxm, thyp, thym, thzp, thzm

  real :: Tboundary = 1.0
 
  real :: tol = 0.1
 
  real :: rho3, rhoE, rhoP, rhoC
        ...
        do j=blkLimits(LOW,JAXIS),blkLimits(HIGH,JAXIS)
         do i=blkLimits(LOW,IAXIS),blkLimits(HIGH,IAXIS)
           ...
           do ib_ind = 1 , ib_stencil
                ...
                solnData(PRES_VAR,ib_external(ib_ind,IAXIS),ib_external(ib_ind,JAXIS),1);
           ...
           do ib_ind = 1 , ib_stencil
                zp(probe_index) = zp(probe_index) + ib_external_phile(ib_ind,CONSTANT_ONE) * &
                solnData(PRES_VAR,ib_external(ib_ind,IAXIS),ib_external(ib_ind,JAXIS),ib_external(ib_ind,KAXIS));
           enddo
           ...
           if(solnData(LMDA_VAR,i,j,k) .lt. 0.0) then
                ...
                rhoC = mph_rho1/mph_rho2
                if(solnData(DFUN_VAR,i,j,k) .lt. 0.0) rhoC = mph_rho2/mph_rho2
                ...
                rhoP = mph_rho1/mph_rho2
                if(phiprobe(1) .lt. 0.0) rhoP = mph_rho2/mph_rho2


           end if

           th = abs(solnData(LMDA_VAR,i,j,k))/(abs(solnData(LMDA_VAR,i,j,k)) + abs(hnorm)) 

           rhoE  = th*rhoC + (1-th)*rhoP

           gravE = ins_gravX*solnData(NMLX_VAR,i,j,k) + &
                   ins_gravY*solnData(NMLY_VAR,i,j,k) + &
                   ins_gravZ*solnData(NMLZ_VAR,i,j,k)

           solnData(PGST_VAR,i,j,k) = zp(1) !- (solnData(LMDA_VAR,i,j,k)+hnorm)*rhoE*gravE
  ...
  gcMask(PGST_VAR) = .TRUE.

  call Grid_fillGuardCells(CENTER_FACES,ALLDIR,&
       maskSize=NUNK_VARS+NDIM*NFACE_VARS,mask=gcMask)

end subroutine mph_applyGFM
