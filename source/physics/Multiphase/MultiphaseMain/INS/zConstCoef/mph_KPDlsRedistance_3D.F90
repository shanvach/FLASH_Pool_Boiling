...
#include "constants.h"
        ...
        real, intent(in) :: lsDT, minCellDiag
           ...
           do j = jy1,jy2
              do i = ix1,ix2
                 ...
                 !- kpd - If cell is NOT near interface...
                   ...
                   ap = max((sm-sxl),0.)*dx(i,LEFT_EDGE)
                   an = min((sm-sxl),0.)*dx(i,LEFT_EDGE)

                   bp = max((sxr-sm),0.)*dx(i,RIGHT_EDGE)
                   bn = min((sxr-sm),0.)*dx(i,RIGHT_EDGE)

                   cp = max((sm-syl),0.)*dy(j,LEFT_EDGE)
                   cn = min((sm-syl),0.)*dy(j,LEFT_EDGE)

                   dp = max((syr-sm),0.)*dy(j,RIGHT_EDGE)
                   dn = min((syr-sm),0.)*dy(j,RIGHT_EDGE)

                   ep = max((sm-szl),0.)*dz(k,LEFT_EDGE)
                   en = min((sm-szl),0.)*dz(k,LEFT_EDGE)

                   fp = max((szr-sm),0.)*dz(k,RIGHT_EDGE)
                   fn = min((szr-sm),0.)*dz(k,RIGHT_EDGE)
