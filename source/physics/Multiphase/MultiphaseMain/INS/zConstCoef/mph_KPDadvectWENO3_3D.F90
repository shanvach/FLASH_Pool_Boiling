         ...
         do k = kz1,kz2 
           do j = jy1,jy2 
              ...
              xcell = coord(IAXIS) - bsize(IAXIS)/2.0 +   &
                      real(i - NGUARD - 1)/dx(i,CENTER) +   &
                      0.5/dx(i,CENTER)

              ycell  = coord(JAXIS) - bsize(JAXIS)/2.0 +  &
                      real(j - NGUARD - 1)/dy(j,CENTER)  +  &
                      0.5/dy(j,CENTER)

              zcell  = coord(KAXIS) - bsize(KAXIS)/2.0 +  &
                      real(k - NGUARD - 1)/dz(k,CENTER)  +  &
                      0.5/dz(k,CENTER)
              ...
              !                        - ins_dampC*AA*(s(i,j,k)-ycell)
              !else
                 s(i,j,k) = so(i,j,k) - dt*(frx*ur - flx*ul)*dx(i,CENTER) &
                                      - dt*(fry*vr - fly*vl)*dy(j,CENTER) &
                                      - dt*(frz*wr - flz*wl)*dz(k,CENTER)
              !end if
              !---------------------------------------------------------------
              !---------------------------------------------------------------
