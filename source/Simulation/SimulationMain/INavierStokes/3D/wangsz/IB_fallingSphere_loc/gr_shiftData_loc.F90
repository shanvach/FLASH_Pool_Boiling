...
subroutine gr_shiftData_loc(comm, dataType, sendRight, sendLeft, &
                        recvRight, recvLeft,nx,ny,nz,force,tmp)
...
#include "Flash_mpi.h"
  ...
  real, dimension(nx,ny,nz), intent(INOUT) :: force
  real, dimension(nx,ny,nz), intent(INOUT) :: tmp
  ...
  call MPI_sendRecv(force(sendRight(2),sendRight(3),sendRight(4)), & 
            &       1, dataType, dest, 1, & 
            &       tmp(recvRight(2),recvRight(3),recvRight(4)), & 
  ...
  call MPI_sendRecv(force(sendLeft(2),sendLeft(3),sendLeft(4)), & 
            &       1, dataType, dest, 1, & 
            &       tmp(recvLeft(2),recvLeft(3),recvLeft(4)), & 
            &       1, dataType, source, 1, comm, status, ierr)  
  
  return
end subroutine gr_shiftData_loc
