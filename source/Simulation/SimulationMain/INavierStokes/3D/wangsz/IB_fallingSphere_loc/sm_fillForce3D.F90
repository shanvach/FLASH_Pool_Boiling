...
subroutine sm_fillForce3D(fX, fY, fZ, tmpX, tmpY, tmpZ)
...
#include "Flash.h"
  ...
  real, dimension(GRID_IHI_GC+1,GRID_JHI_GC,GRID_KHI_GC), intent(inout) :: fX
  real, dimension(GRID_IHI_GC,GRID_JHI_GC+1,GRID_KHI_GC), intent(inout) :: fY
  real, dimension(GRID_IHI_GC,GRID_JHI_GC,GRID_KHI_GC+1), intent(inout) :: fZ
  real, dimension(GRID_IHI_GC+1,GRID_JHI_GC,GRID_KHI_GC), intent(inout) :: tmpX
  real, dimension(GRID_IHI_GC,GRID_JHI_GC+1,GRID_KHI_GC), intent(inout) :: tmpY
  real, dimension(GRID_IHI_GC,GRID_JHI_GC,GRID_KHI_GC+1), intent(inout) :: tmpZ

!  real, dimension(GRID_IHI_GC+1,GRID_JHI_GC,GRID_KHI_GC):: tmpLocX
!  real, dimension(GRID_IHI_GC,GRID_JHI_GC+1,GRID_KHI_GC):: tmpLocY
!  real, dimension(GRID_IHI_GC,GRID_JHI_GC,GRID_KHI_GC+1):: tmpLocZ
...
!  tmpLocZ = 0.
  ...
  sendRight(3,4) = blkLimits(HIGH,KAXIS) + 1
  sendLeft(3,4)  = blkLimits(LOW,KAXIS) - 2

  recvRight(3,4)  = blkLimits(LOW,KAXIS)
  recvLeft(3,4)   = blkLimits(HIGH,KAXIS) - 1

  call gr_shiftData_loc(gr_axisComm(KAXIS), gr_exchLoc(FACEX_DATATYPE,KAXIS), & 
                  &     sendRight(KAXIS,:), sendLeft(KAXIS,:),  & 
                  &     recvRight(KAXIS,:), recvLeft(KAXIS,:),  & 
                  &     GRID_IHI_GC+1, GRID_JHI_GC, GRID_KHI_GC, fX, tmpX)
  ...
  call gr_shiftData_loc(gr_axisComm(KAXIS), gr_exchLoc(FACEY_DATATYPE,KAXIS), & 
                  &     sendRight(KAXIS,:), sendLeft(KAXIS,:),  & 
                  &     recvRight(KAXIS,:), recvLeft(KAXIS,:),  & 
  ...
  call gr_shiftData_loc(gr_axisComm(JAXIS), gr_exchLoc(FACEZ_DATATYPE,JAXIS), & 
                  &     sendRight(JAXIS,:), sendLeft(JAXIS,:),  & 
                  &     recvRight(JAXIS,:), recvLeft(JAXIS,:),  & 
                  &     GRID_IHI_GC, GRID_JHI_GC, GRID_KHI_GC+1, fZ,tmpZ)

  fZ = fZ + tmpZ
  tmpZ = 0.

  sendRight(3,4) = blkLimits(HIGH,KAXIS) 
  sendLeft(3,4)  = blkLimits(LOW,KAXIS) - 1

  recvRight(3,4)  = blkLimits(LOW,KAXIS)
  recvLeft(3,4)   = blkLimits(HIGH,KAXIS) - 1

  call gr_shiftData_loc(gr_axisComm(KAXIS), gr_exchLoc(FACEZ_DATATYPE,KAXIS), & 
                  &     sendRight(KAXIS,:), sendLeft(KAXIS,:),  & 
                  &     recvRight(KAXIS,:), recvLeft(KAXIS,:),  & 
                  &     GRID_IHI_GC, GRID_JHI_GC, GRID_KHI_GC+1, fZ,tmpZ)
 
  if(.not.gr_allPeriodic) then
    call gr_bcApplyToAllBlks_loc3D(tmpX, tmpY, tmpZ)
  end if

  return
end subroutine sm_fillForce3D
