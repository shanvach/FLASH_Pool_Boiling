...
#include "Flash.h"
  ...
  do i = 1,gr_numDataStruct
     ...
     if(NDIM==1) then
        ...
        ! For local variables , Shizhao Wang, Jan 2015 
        ! y exchange
        ! for up and down, ydir
        call MPI_TYPE_CONTIGUOUS(guard(JAXIS)*&
             blkExtent(IAXIS), & 
             FLASH_REAL, exch2Loc, ierr)
        call MPI_TYPE_COMMIT(exch2Loc,ierr)
        
        
        ! for left and right, xdir
        ! vector is needed to simplify exchange
        ! call MPI_TYPE_VECTOR(count, blocklength, stride, oldtype, newtype)
        call MPI_TYPE_VECTOR(blkExtent(JAXIS), guard(IAXIS), &
             (blkExtent(IAXIS)), &
             FLASH_REAL, exch1Loc, ierr)
        
        call MPI_TYPE_COMMIT(exch1Loc, ierr)
        ...
        if(NDIM == 2) then
           ...
           gr_exchLoc(i,IAXIS)=exch1Loc
           gr_exchLoc(i,JAXIS)=exch2Loc
        else
           ...
           ! for z exchange , Shizhao Wang, Jan 28, 2015
           ! for left and right, xdir
           ! using an hvector 
           ...
           call MPI_TYPE_HVECTOR(blkExtent(KAXIS), 1,  &
                ...
                flashRealExtent, & 
                exch1Loc, gr_exchLoc(i,IAXIS), ierr)
           
           call MPI_TYPE_COMMIT(gr_exchLoc(i,IAXIS),ierr)
           ...
           call MPI_TYPE_HVECTOR(blkExtent(KAXIS), 1,  &
                blkExtent(IAXIS)*blkExtent(JAXIS)*&
                flashRealExtent, & 
                exch2Loc, gr_exchLoc(i,JAXIS), ierr)
           
           call MPI_TYPE_COMMIT(gr_exchLoc(i,JAXIS),ierr)
           
           
           ! for back and forth, zdir
           ! contiguous in memory
           size = guard(KAXIS) * blkExtent(IAXIS) * blkExtent(JAXIS)
           call MPI_TYPE_CONTIGUOUS(size, FLASH_REAL, gr_exchLoc(i,KAXIS), ierr)
           call MPI_TYPE_COMMIT(gr_exchLoc(i,KAXIS),ierr)
