...
! Project external and marker Velocity on the plane:
  ...
  !if( sqrt(dun**2+dvn**2+dwn**2) .lt. 1.0e-12) then ! case zero velocity difference
  if( sqrt(dun**2+dvn**2+dwn**2) .lt. 1.0e-8) then ! case zero velocity difference
    tp = 0.
  else
  ...
  umag = sqrt(ubd*ubd+vbd*vbd+wbd*wbd)

!  if( sqrt(ues**2+ves**2+wes**2) .lt. 1.0e-12) then ! case zero velocity difference
!    tp = 0.
!  else
!   normt = 1./sqrt(ues**2. + ves**2. + wes**2.)
!   tp(1) = ues*normt
!   tp(2) = ves*normt
!   tp(3) = wes*normt
!  endif
  ...
  tmp_dv = dudx*np(1)*tp(1) + dvdx*np(1)*tp(2) + dwdx*np(1)*tp(3) + &
         & dudy*np(2)*tp(1) + dvdy*np(2)*tp(2) + dwdy*np(2)*tp(3) + &
         & dudz*np(3)*tp(1) + dvdz*np(3)*tp(2) + dwdz*np(3)*tp(3) 
...
!  particleData(FZVI_PART_PROP) = fvz_l + fvz_c + fvz_cc
  particleData(FXVI_PART_PROP) = fvx_l + (fvx_c + fvx_cc)*(2.0/pi)*atan(10.0*umag/nu)
  particleData(FYVI_PART_PROP) = fvy_l + (fvy_c + fvy_cc)*(2.0/pi)*atan(10.0*umag/nu)
  particleData(FZVI_PART_PROP) = fvz_l + (fvz_c + fvz_cc)*(2.0/pi)*atan(10.0*umag/nu)
  ...
  write(9700+gr_meshMe,'(30f20.12)') particleData(GLOB_PART_PROP), particleData(FZVI_PART_PROP), fvz_l, fvz_c, fvz_cc
  write(9600+gr_meshMe,'(30f20.12)') particleData(GLOB_PART_PROP), particleData(FYVI_PART_PROP), fvy_l, fvy_c, fvy_cc
  write(9500+gr_meshMe,'(30f20.12)') particleData(GLOB_PART_PROP), particleData(FXVI_PART_PROP), fvx_l, fvx_c, fvx_cc
...
#ifdef TEST_CHECK_GRAD
  write(*,*) 'test viscous forces'
!  particleData(VORX_PART_PROP) = fvx_c
!  particleData(VORY_PART_PROP) = fvy_c
!  particleData(VORZ_PART_PROP) = fvz_c

!  particleData(NMLX_PART_PROP) = fvx_cc
!  particleData(NMLY_PART_PROP) = fvy_cc
!  particleData(NMLZ_PART_PROP) = fvz_cc

!  particleData(VORX_PART_PROP) = tp(1)
!  particleData(VORY_PART_PROP) = tp(2)
!  particleData(VORZ_PART_PROP) = tp(3)

!  particleData(FULO_PART_PROP) = tmp_dv
!  particleData(FVLO_PART_PROP) = tmp_ddv
!  particleData(FWLO_PART_PROP) = fvz_l

  particleData(POSX_PART_PROP) = xbe(1)
  particleData(POSY_PART_PROP) = xbe(2)
  particleData(POSZ_PART_PROP) = xbe(3)

  particleData(NMLX_PART_PROP) = np(1)
  particleData(NMLY_PART_PROP) = np(2)
  particleData(NMLZ_PART_PROP) = np(3)

  particleData(VORX_PART_PROP) = tp(1) 
  particleData(VORY_PART_PROP) = tp(2)
  particleData(VORZ_PART_PROP) = tp(3)

  particleData(VELX_PART_PROP) = ue
  particleData(VELY_PART_PROP) = ve
  particleData(VELZ_PART_PROP) = we

  particleData(FUL_PART_PROP) = ues
  particleData(FVL_PART_PROP) = ves
  particleData(FWL_PART_PROP) = wes

  particleData(FULO_PART_PROP) = ven*nxp
  particleData(FVLO_PART_PROP) = ven*nyp
  particleData(FWLO_PART_PROP) = ven*nzp

  particleData(ACCX_PART_PROP) = tmp_dv
  particleData(ACCY_PART_PROP) = tmp_ddv
  particleData(ACCZ_PART_PROP) = tmp_dddv
#endif
  return
