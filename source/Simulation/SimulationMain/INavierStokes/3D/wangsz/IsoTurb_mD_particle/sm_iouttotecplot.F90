...
!        enddo
        ...
        if (any(sm_BodyInfo(ibd)%OnBoundary(:,:) .eqv. .TRUE.) .and. .true.) then
           ...
           elseif(x_sgn .eq. -1) then 
              ...
              nwx = int(abs(minval(vx(:)-eps)-gr_globalDomain(LOW,IAXIS))/Lx)
           endif
           ...
           elseif(y_sgn .eq. -1) then 
              ...
              nwy = int(abs(minval(vy(:)-eps)-gr_globalDomain(LOW,JAXIS))/Ly)
           endif
           ...
           elseif(z_sgn .eq. -1) then 
              ...
              nwz = int(abs(minval(vz(:)-eps)-gr_globalDomain(LOW,KAXIS))/Lz) 
           endif
#endif           
           plot_nel = 0
