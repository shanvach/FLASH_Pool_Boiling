...
subroutine sm_surf_assembleFluidForce_toPoint(ibd, point, force_pres, force_visc, moment_pres, moment_visc)
  ...
  do p = 1,gr_sbBodyInfo(ibd)%totalPart
     ...
     do while (newRpcm(1) > sm_CL)  
         newRpcm(1) = newRpcm(1) - Lx
     enddo
     do while (newRpcm(1) < -sm_CL)  
         newRpcm(1) = newRpcm(1) + Lx
     enddo

     do while (newRpcm(2) > sm_CL)  
         newRpcm(2) = newRpcm(2) - Ly
     enddo
     do while (newRpcm(2) < -sm_CL)  
         newRpcm(2) = newRpcm(2) + Ly
     enddo
     ...
     if(rr > sm_CL) then
       ...
       write(*,*) 'Force arm length:', rr, p, newPoint, pos(1:NDIM)       
     endif
     rpcm = newRpcm
     ...
     do while (newRpcm(3) > sm_CL)  
         newRpcm(3) = newRpcm(3) - Lz
     enddo
     do while (newRpcm(3) < -sm_CL)  
         newRpcm(3) = newRpcm(3) + Lz
     enddo
     ...
     if(rr > sm_CL) then
       !call Driver_abortFlash('Error in calculating the moment')
       write(*,*) 'Force arm length:', rr, p, newPoint, pos(1:NDIM), ibd, sm_CL, Lx, Ly, Lz, newRpcm, rpcm       
     endif
     rpcm = newRpcm
