...
!#define CONST_INPUT_ENG
  ...
  use SolidMechanics_data, ONLY : sm_volFrac
...
#include "IncompNS.h"
  ...
  do lb = 1,blockCount
     ...
     uavg = uavg + &
     sum(facexData(VELC_FACE_VAR,ng+2:nxc,ng+1:nyj,ng+1:nzk)* &
         facexData(IBLK_FACE_VAR,ng+2:nxc,ng+1:nyj,ng+1:nzk))*volcell
 
    ! V average:
    !vavg = vavg + &
    ! sum(faceyData(VELC_FACE_VAR,ng+1:nxi,ng+2:nyc,ng+1:nzk))*volcell
    vavg = vavg + &
    sum(faceyData(VELC_FACE_VAR,ng+1:nxi,ng+2:nyc,ng+1:nzk)* &
        faceyData(IBLK_FACE_VAR,ng+1:nxi,ng+2:nyc,ng+1:nzk))*volcell
  
    ! W average:
    !wavg = wavg + &
    ! sum(facezData(VELC_FACE_VAR,ng+1:nxi,ng+1:nyj,ng+2:nzc))*volcell
    wavg = wavg + &
     sum(facezData(VELC_FACE_VAR,ng+1:nxi,ng+1:nyj,ng+2:nzc)* &
         facezData(IBLK_FACE_VAR,ng+1:nxi,ng+1:nyj,ng+2:nzc))*volcell
  ...
  tvol = tvol*(1.-sm_volFrac)  ! Shizhao for particle-laden flows
...
!!$           facevarz(1,ng+1:nxi,ng+1:nyj,ng+2:nzc,lb))**2 )               
      ...
      turbkin = turbkin + & 
         sum(solnData(DUST_VAR,ng+1:nxi,ng+1:nyj,ng+1:nzk)*      &
             solnData(IBLK_VAR,ng+1:nxi,ng+1:nyj,ng+1:nzk))*volcell
  ...
  if (mype .eq. 0) then
     ...
     if (initfile) then
      ...
      OPEN(UNIT=241,FILE='./IOData/TURBKINLAM.res',STATUS='OLD',&
           FORM='formatted',POSITION='APPEND')
      write(241,'(I16,6g20.12)')istep,time,turbkin,lambdaf,lambdag,dissavg,sim_scaleForce*sim_fbao
      write(*,*) &
      'ISTEP      TIME      TURBKIN        Lambdaf       Lambdag        dissavg'
      write(*,'(I8,5g14.6)')istep,time,turbkin,lambdaf,lambdag,dissavg
      close(241) 
