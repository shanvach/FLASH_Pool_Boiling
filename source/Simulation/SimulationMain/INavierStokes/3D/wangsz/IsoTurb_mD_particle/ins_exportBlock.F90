...
#include "IncompNS.h"
  ...
  do lb = 1,blockCount
      ...
      open(iFile,file=trim(filename),status='replace',form='unformatted')
        ...
        write(iFile)  (((facexData(VELO_FACE_VAR,i,j,k), i=ixs,ixe+1),j=iys,iye),k=izs,ize) 
        write(iFile)  (((faceyData(VELO_FACE_VAR,i,j,k), i=ixs,ixe),j=iys,iye+1),k=izs,ize) 
        write(iFile)  (((facezData(VELO_FACE_VAR,i,j,k), i=ixs,ixe),j=iys,iye),k=izs,ize+1) 

        write(iFile)  (((solnData(IBLK_VAR,i,j,k), i=ixs,ixe),j=iys,iye),k=izs,ize) 
        write(iFile)  (((facexData(IBLK_FACE_VAR,i,j,k), i=ixs,ixe+1),j=iys,iye),k=izs,ize) 
        write(iFile)  (((faceyData(IBLK_FACE_VAR,i,j,k), i=ixs,ixe),j=iys,iye+1),k=izs,ize) 
        write(iFile)  (((facezData(IBLK_FACE_VAR,i,j,k), i=ixs,ixe),j=iys,iye),k=izs,ize+1) 
