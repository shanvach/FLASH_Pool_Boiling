...
subroutine SolidMechanics_init(restart)
  ...
  use SolidMechanics_data, only : sm_MeshMe,sm_NumProcs, sm_NumBodies, sm_BodyInfo, sm_meshComm, &
                                  ...
                                  sm_gravX, sm_gravY, sm_gravZ, sm_pos, sm_volFrac, sm_CL
...
!    call sm_set_cm_para()
  ...
  ! volume fraction Vol_solid/Vol_fluid
  call RuntimeParameters_get("sm_volFrac", sm_volFrac)
  call RuntimeParameters_get("sm_CL", sm_CL)
  if(sm_meshMe == MASTER_PE) then
    write(*,*) 'volume fraction in the simulation:', sm_volFrac
    write(*,*) 'Critical length of particles:', sm_CL
  endif
