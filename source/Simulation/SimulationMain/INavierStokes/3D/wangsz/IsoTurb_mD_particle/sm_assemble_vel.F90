...
! Modified by Shizhao Wang to add the IBLK_FACE_VAR for particle-laden
! turbulence
! Apr 12, 2015
 
subroutine sm_assemble_vel()
...
#include "SolidMechanics.h"
  ...
  do ibd = 1,sm_NumBodies
     if ( (sm_MeshMe .eq. sm_BodyInfo(ibd)%BodyMaster) .and. & 
        ...
        !write(*,*) 'r_bod', r_bod(:,ibd), 'ibd', ibd, 'Master', sm_meshMe
     endif
  enddo
  ...
  do lb = 1,blockCount
...
#endif
     solnData(IBLK_VAR,:,:,:) = 1.0
     facexData(IBLK_FACE_VAR,:,:,:) = 1.0
     faceyData(IBLK_FACE_VAR,:,:,:) = 1.0
     facezData(IBLK_FACE_VAR,:,:,:) = 1.0
     ...
     do ibd = 1,sm_NumBodies
        ...
        case ( RB_ANNSPHERE )
           ...
           do k=blkLimits(LOW,KAXIS),blkLimits(HIGH,KAXIS)
              do j=blkLimits(LOW,JAXIS),blkLimits(HIGH,JAXIS)
                 do i=blkLimits(LOW,IAXIS),blkLimits(HIGH,IAXIS)
                       ...
                       if ( vecnorm .lt. (radius+hcell) ) then
                          ...
                          facezData(IBLK_FACE_VAR,i,j,k) = 1.-alphaijk
              ...
              enddo
           enddo
           facezData(IBLK_FACE_VAR,:,:,NGUARD+NZB+1) = facezData(IBLK_FACE_VAR,:,:,NGUARD+NZB) 
           ...
           do k=blkLimits(LOW,KAXIS),blkLimits(HIGH,KAXIS)
              do j=blkLimits(LOW,JAXIS),blkLimits(HIGH,JAXIS)
                 do i=blkLimits(LOW,IAXIS),blkLimits(HIGH,IAXIS)
                       ...
                       if ( vecnorm .lt. (radius+hcell) ) then
                          ...
                          faceyData(IBLK_FACE_VAR,i,j,k) = 1.-alphaijk
              ...
              enddo
           enddo
           faceyData(IBLK_FACE_VAR,:,NGUARD+NYB+1,:) = faceyData(IBLK_FACE_VAR,:,NGUARD+NYB,:) 
           ...
           do k=blkLimits(LOW,KAXIS),blkLimits(HIGH,KAXIS)
              do j=blkLimits(LOW,JAXIS),blkLimits(HIGH,JAXIS)
                 do i=blkLimits(LOW,IAXIS),blkLimits(HIGH,IAXIS)
                       ...
                       if ( vecnorm .lt. (radius+hcell) ) then
                          ...
                          facexData(IBLK_FACE_VAR,i,j,k) = 1.-alphaijk
              ...
              enddo
           enddo
           facexData(IBLK_FACE_VAR,NGUARD+NXB+1,:,:) = facexData(IBLK_FACE_VAR,NGUARD+NXB,:,:) 
           ...
           do k=blkLimits(LOW,KAXIS),blkLimits(HIGH,KAXIS)
              do j=blkLimits(LOW,JAXIS),blkLimits(HIGH,JAXIS)
                 do i=blkLimits(LOW,IAXIS),blkLimits(HIGH,IAXIS)
                       ...
                       ! Distance from body center of mass to X veloc point
                       vec(1:MDIM) = TBN_xcell(1:MDIM,i) + TBN_ycell(1:MDIM,j) + TBN_zcell(1:MDIM,k)
                       vecnorm = sqrt(vec(IAXIS)**2. + vec(JAXIS)**2. + vec(KAXIS)**2.)
                       ...
                       if ( vecnorm .lt. (radius+hcell) ) then
                          ...
                          solnData(IBLK_VAR,i,j,k) = 1.-alphaijk
        ...
        case ( RB_ANNDISC )
           ...
           write(*,*) 'The IBLK_FACE_VAR is not tested for the RB_ANNDISC'
           stop
     ...
     facexData(VELO_FACE_VAR,:,:,:) = facexData(VELC_FACE_VAR,:,:,:) * facexData(IBLK_FACE_VAR,:,:,:)
     faceyData(VELO_FACE_VAR,:,:,:) = faceyData(VELC_FACE_VAR,:,:,:) * faceyData(IBLK_FACE_VAR,:,:,:)
     facezData(VELO_FACE_VAR,:,:,:) = facezData(VELC_FACE_VAR,:,:,:) * facezData(IBLK_FACE_VAR,:,:,:)
  ...
  do ibd = 1,sm_NumBodies
     if ( (sm_MeshMe .eq. sm_BodyInfo(ibd)%BodyMaster) .and. &
        ...
        !write(*,*) 'Volumes X,Y,ANN=',int_tvol(IAXIS,ibd),int_tvol(JAXIS,ibd),body_vol
  ...
  return
end subroutine sm_assemble_vel
