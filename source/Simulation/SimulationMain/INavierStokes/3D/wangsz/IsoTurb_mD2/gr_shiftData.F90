...
subroutine gr_shiftData(comm, dataType, sendRight, sendLeft, &
  ...
  use Grid_data, ONLY : gr_sendBuf, gr_bufSize, gr_meshMe
...
#include "Flash_mpi.h"
  ...
  write(*,*) 'locMe, locSize, comm, grMe', localMyPE, localNumProcs, comm, gr_meshMe 
...
#ifdef USE_BUFF_COMM
  call MPI_Buffer_attach(gr_sendBuf, gr_bufSize, ierr)
  select case(gridDataStruct)
  case(CENTER) 
     call MPI_BSend(unk(sendRight(1), sendRight(2), &
          sendRight(3), sendRight(4),1), &
          1, dataType, dest, 1,&
          comm, ierr)
     call MPI_Recv(unk(recvRight(1), recvRight(2), &
          recvRight(3), recvRight(4),1), &
          1, dataType, source, 1, comm, status, ierr)
  case(FACEX) 
     call MPI_BSend(facevarx(sendRight(1), sendRight(2), &
          sendRight(3), sendRight(4),1), &
          1, dataType, dest, 1,&
          comm, ierr)
     call MPI_Recv(facevarx(recvRight(1), recvRight(2), &
          recvRight(3), recvRight(4),1), &
          1, dataType,source, 1, comm, status, ierr)
     
  case(FACEY) 
     call MPI_BSend(facevary(sendRight(1), sendRight(2), &
          sendRight(3), sendRight(4),1), &
          1, dataType, dest, 1,&
          comm, ierr)
     call MPI_Recv(facevary(recvRight(1), recvRight(2), &
          recvRight(3), recvRight(4),1), &
          1, dataType,source, 1, comm, status, ierr)
     
  case(FACEZ) 
     call MPI_BSend(facevarz(sendRight(1), sendRight(2), &
          sendRight(3), sendRight(4),1), &
          1, dataType, dest, 1,&
          comm, ierr)
     call MPI_Recv(facevarz(recvRight(1), recvRight(2), &
          recvRight(3), recvRight(4),1), &
          1, dataType,source, 1, comm, status, ierr)
     
  end select
  call MPI_Buffer_detach(gr_sendBuf, gr_bufSize, ierr)
#else
  select case(gridDataStruct)
  case(CENTER) 
