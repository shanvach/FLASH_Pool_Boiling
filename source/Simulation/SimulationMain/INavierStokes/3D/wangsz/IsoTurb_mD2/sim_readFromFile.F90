!! Read the flow filed from a restart file
!! Based on the file as follows
...
subroutine sim_readFromFile(filename)
  ...
  use IO_data, ONLY : io_globalMe, io_globalNumProcs,io_globalComm,&
       ...
       io_numRealParms, io_numIntParms, io_numStrParms, io_numLogParms, io_prevParmPad, io_maxParms
...
! number.  Note this is set to be 4 characters long (i.e. max = 9999).  
  ...
  if (io_globalMe == MASTER_PE) then
     ...
     write (*,*) '[sim_readFromFile] Opening ', trim(filename), &
          ' for initial flow'
...
!     deallocate(strBuf)
     ...
     write(*,*) 'read header:', io_globalMe, io_chkptFileID, io_unklabels, io_outputSplitNum 

     !allocate space for scalar and parameter lists
     call io_prepareListsRead()
     write(*,*) 'prepareListRead'
...
end subroutine sim_readFromFile
