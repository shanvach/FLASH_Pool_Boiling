...
!  body%Hs_pres = 0.0
!  body%Hs_visc = 0.0
!  write(*,*) 'debug: set Hs_pres and Hs_visc to 0'
  ...
  do i = body%ix,body%ex
     ...
     if (idx .le. Body%neq) then
        ...
        !body%Hs(idx)  = body%Hs(idx)/2. !+ body%Hs_pres(idx) + body%Hs_visc(idx)
     endif
  end do
  ...
  do i = body%iw,body%ew
     ...
     if (idx .le. Body%neq) then
        body%Hs(idx)  = body%Hs(idx) + body%Hs_pres(idx) + body%Hs_visc(idx)
        !body%Hs(idx)  = body%Hs(idx)/2.0 ! + body%Hs_pres(idx) + body%Hs_visc(idx)
     endif
  end do
