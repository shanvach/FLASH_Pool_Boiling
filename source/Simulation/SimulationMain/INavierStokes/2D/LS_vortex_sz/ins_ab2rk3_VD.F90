...
! --------- ---
  do lb = 1,blockCount
   ...
   !if(ins_meshMe == 0) write(*,*) 'block size:', bsize

    do j=blkLimits(LOW,JAXIS),blkLimits(HIGH,JAXIS)
       do i=blkLimits(LOW,IAXIS), blkLimits(HIGH,IAXIS)+1

          xface = coord(IAXIS) - bsize(IAXIS)/2.0 +   &
     ...
     do j=blkLimits(1,JAXIS),blkLimits(2,JAXIS)+1
       do i=blkLimits(1,IAXIS), blkLimits(2,IAXIS)
          ...
          yface  = coord(JAXIS) - bsize(JAXIS)/2.0 +  &
                  real(j - NGUARD - 1)*del(JAXIS)
 
          !faceyData(VELC_FACE_VAR,i,j,1) = xcell*2.0d0*acos(-1.0d0)
          !faceyData(VELC_FACE_VAR,i,j,1) = 0.0d0
