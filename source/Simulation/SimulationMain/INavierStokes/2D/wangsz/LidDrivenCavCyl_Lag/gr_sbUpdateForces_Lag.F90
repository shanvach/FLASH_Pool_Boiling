...
subroutine gr_sbUpdateForces_Lag
...
#include "Flash_mpi.h"
  ...
  real, allocatable, dimension(:,:) :: particles3
  ...
  do b = 1, gr_sbNumBodies
     ...
     call Timers_start("InitialParticles2")
     particles2(:,:) = 0.
#ifdef FLASH_GRID_PARAMESH
...
#endif
     call Timers_stop("InitialParticles2")


     call Timers_start("gr_setIndices")
     !! Now get all the indices into the data structure setup right
     !! for the rest of the unit
     call gr_ptSetIndices(pt_indexList, pt_indexCount)
     call Timers_stop("gr_setIndices")

     if (bodyInfo % myPE == bodyInfo % bodyMaster) then

#ifdef TEST_SET_PARTICLES
        call Timers_start('TestSetParticles2')
          do i = 1, pt_maxPerProc
            particles2(:,i) = i
          enddo
        call Timers_stop('TestSetParticles2')
        call Timers_start('Test2SetParticles2')
            particles2 = 0.
        call Timers_stop('Test2SetParticles2')
        call Timers_start('TestAccessParticle')
          do i = 1, totalPart
            tmp(:) = bodyInfo%particles(:,i)
          enddo
        call Timers_stop('TestAccessParticle')

        allocate(particles3(1:NPART_PROPS,bodyInfo%totalPart))
        call Timers_start('TestAccessParticle3')
          do i = 1, totalPart
            particles3(:,i) = bodyInfo%particles(:,i)
          enddo
        call Timers_stop('TestAccessParticle3')

        call Timers_start('Test2AccessParticle3')
          particles3 = bodyInfo%particles
        call Timers_stop('Test2AccessParticle3')
        call Timers_start('Test3SetParticles2')
          do i = 1, pt_maxPerProc
            particles2(:,i) = particles3(:,i)
          enddo
        call Timers_stop('Test3SetParticles2')
...
! Turn off to test the parallel performance
...
end subroutine gr_sbUpdateForces_Lag
