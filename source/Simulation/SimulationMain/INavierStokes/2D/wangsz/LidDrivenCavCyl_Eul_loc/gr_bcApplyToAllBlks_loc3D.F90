...
!!
!!***
subroutine gr_bcApplyToAllBlks_loc3D(tmpX,tmpY,tmpZ)
...
#include "Flash.h"
  ...
  if(blkBC(LOW,JAXIS)/=NOT_BOUNDARY) then
    ...
    tmpZ(:,GRID_JLO:GRID_JLO+1,:) = 0.
  endif
  if(blkBC(HIGH,JAXIS)/=NOT_BOUNDARY) then
    tmpX(:,GRID_JHI-1:GRID_JHI,:) = 0.
    tmpY(:,GRID_JHI-1:GRID_JHI,:) = 0.
    tmpZ(:,GRID_JHI-1:GRID_JHI,:) = 0.
  endif
  if(blkBC(LOW,KAXIS)/=NOT_BOUNDARY) then
    tmpX(:,:,GRID_KLO:GRID_KLO+1) = 0.
    tmpY(:,:,GRID_KLO:GRID_KLO+1) = 0.
    tmpZ(:,:,GRID_KLO:GRID_KLO+1) = 0.
  endif
  if(blkBC(HIGH,KAXIS)/=NOT_BOUNDARY) then
    tmpX(:,:,GRID_KHI-1:GRID_KHI) = 0.
    tmpY(:,:,GRID_KHI-1:GRID_KHI) = 0.
    tmpZ(:,:,GRID_KHI-1:GRID_KHI) = 0.
  endif

return  
end subroutine gr_bcApplyToAllBlks_loc3D
