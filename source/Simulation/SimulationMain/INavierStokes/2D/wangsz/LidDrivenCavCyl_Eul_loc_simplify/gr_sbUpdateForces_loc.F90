...
subroutine gr_sbUpdateForces_loc
...
#ifdef FLASH_GRID_PARAMESH
#ifdef USE_LOC_VAR
  write(*,*) 'The code with USE_LOC_VAR has not been tested for FLASH_GRID_PARAMESH'
  stop
#endif
#endif
...
#ifdef USE_LOC_VAR
  real :: forceX(GRID_IHI_GC+1,GRID_JHI_GC  ,GRID_KHI_GC)
  real :: forceY(GRID_IHI_GC,GRID_JHI_GC+1  ,GRID_KHI_GC)
  real :: tmpX(GRID_IHI_GC+1,GRID_JHI_GC  ,GRID_KHI_GC)
  real :: tmpY(GRID_IHI_GC,GRID_JHI_GC+1  ,GRID_KHI_GC)
#if NDIM == MDIM
  real :: forceZ(GRID_IHI_GC,GRID_JHI_GC,GRID_KHI_GC+1)
  real :: tmpZ(GRID_IHI_GC,GRID_JHI_GC,GRID_KHI_GC+1)
#endif
#endif

#ifdef USE_LOC_VAR_TEST
  real :: FxFixed3D(GRID_IHI_GC+1,GRID_JHI_GC  ,GRID_KHI_GC)
  real :: FyFixed3D(GRID_IHI_GC,GRID_JHI_GC+1  ,GRID_KHI_GC)
  real :: FxFixed1D((GRID_IHI_GC+1)*GRID_JHI_GC*GRID_KHI_GC)
  real :: FyFixed1D(GRID_IHI_GC*(GRID_JHI_GC+1)*GRID_KHI_GC)
  real, allocatable, dimension(:,:,:) :: FxAssumed3D, FyAssumed3D
  real, allocatable, dimension(:) :: FxAssumed1D, FyAssumed1D

  allocate(FxAssumed3D(GRID_IHI_GC+1,GRID_JHI_GC,GRID_KHI_GC))
  allocate(FyAssumed3D(GRID_IHI_GC,GRID_JHI_GC+1,GRID_KHI_GC))

  allocate(FxAssumed1D((GRID_IHI_GC+1)*GRID_JHI_GC*GRID_KHI_GC))
  allocate(FyAssumed1D((GRID_IHI_GC)*(GRID_JHI_GC+1)*GRID_KHI_GC))

  call Timers_start('ArrayFixed3D')
  !call test_subArray(FxFixed3D, FyFixed3D)
  call test_subArray(FxFixed3D, FyFixed3D, GRID_IHI_GC, GRID_JHI_GC, GRID_KHI_GC)
  call Timers_stop('ArrayFixed3D')

  call Timers_start('ArrayAssumed3D')
  !call test_subArray(FxAssumed3D, FyAssumed3D)
  call test_subArray(FxAssumed3D, FyAssumed3D, GRID_IHI_GC, GRID_JHI_GC, GRID_KHI_GC)
  call Timers_stop('ArrayAssumed3D')

  call Timers_start('ArrayFixed1D')
  !call test_subArray1D(FxFixed1D, FyFixed1D)
  call test_subArray1D(FxFixed1D, FyFixed1D, GRID_IHI_GC, GRID_JHI_GC, GRID_KHI_GC)
  call Timers_stop('ArrayFixed1D')

  call Timers_start('ArrayAssumed1D')
  !call test_subArray1D(FxAssumed1D, FyAssumed1D)
  call test_subArray1D(FxAssumed1D, FyAssumed1D, GRID_IHI_GC, GRID_JHI_GC, GRID_KHI_GC)
  call Timers_stop('ArrayAssumed1D')
#endif

#ifdef USE_LOC_VAR_TEST
  call Timers_start('CopyToLocal')
    call sm_copyToLocal(FORC_FACE_VAR,forceX,GRID_IHI_GC+1,GRID_JHI_GC,GRID_KHI_GC,FACEX)
    call sm_copyToLocal(FORC_FACE_VAR,forceY,GRID_IHI_GC,GRID_JHI_GC+1,GRID_KHI_GC,FACEY)
  call Timers_stop('CopyToLocal')

  call Timers_start('InitEulerLocal')
    forceX = 0.
    forceY = 0.
  call Timers_stop('InitEulerLocal')

  call Timers_start('CopyToGlobal')
    call sm_copyToGlobal(FORC_FACE_VAR,forceX,GRID_IHI_GC+1,GRID_JHI_GC,GRID_KHI_GC,FACEX)
    call sm_copyToGlobal(FORC_FACE_VAR,forceY,GRID_IHI_GC,GRID_JHI_GC+1,GRID_KHI_GC,FACEY)
  call Timers_stop('CopyToGlobal')
#endif
...
#ifdef USE_LOC_VAR
#if NDIM == MDIM
              call ib_forcing_loc3D(b,int(particleData(GLOB_PART_PROP)),blockID, particleData, forceX, forceY, forceZ)
#else
              call ib_forcing_loc2D(b,int(particleData(GLOB_PART_PROP)),blockID, particleData, forceX, forceY)
#endif
#else
...
!              blockID = int(bodyInfo % particles(BLK_PART_PROP,p))
...
#ifdef USE_LOC_VAR
#if NDIM == MDIM
              call ib_forcing_loc3D(b,p,blockID, particleData, forceX, forceY, forceZ)
#else
              call ib_forcing_loc2D(b,p,blockID, particleData, forceX, forceY)
#endif
#else
 ...
 do lb = 1, GRID_IHI_GC+1
     
   if(abs(forceX(lb,2,1))>1.0d-12 .or. abs(forceX(lb,GRID_JHI_GC-1,1))>1.0d-12) then
   write(1000000+gr_meshMe,'(4f20.12)') forceX(lb,1,1),forceX(lb,2,1),forceX(lb,GRID_JHI_GC-1,1),forceX(lb,GRID_JHI_GC,1)
   endif
   if(abs(forceX(lb,1,1))>1.0d-12) then
     write(1100000+gr_meshMe,*) forceX(lb,1,1)
   endif  
   if(abs(forceX(lb,GRID_JHI_GC,1))>1.0d-12) then
     write(1100000+gr_meshMe,*) forceX(lb,GRID_JHI_GC,1)
   endif  
 enddo

 do lb = 1, GRID_IHI_GC
     
   if(abs(forceY(lb,2,1))>1.0d-12 .or. abs(forceY(lb,GRID_JHI_GC,1))>1.0d-12) then
   write(2000000+gr_meshMe,'(4f20.12)') forceY(lb,1,1),forceY(lb,2,1),forceY(lb,GRID_JHI_GC,1),forceX(lb,GRID_JHI_GC+1,1)
   endif
   if(abs(forceY(lb,1,1))>1.0d-12) then
     write(2100000+gr_meshMe,*) forceY(lb,1,1)
   endif  
   if(abs(forceY(lb,GRID_JHI_GC+1,1))>1.0d-12) then
     write(2100000+gr_meshMe,*) forceY(lb,GRID_JHI_GC+1,1)
   endif  
 enddo
#endif
...
#if NDIM == MDIM
 call sm_fillForce3D(forceX, forceY, forceZ, tmpX, tmpY, tmpZ)
#else
 call sm_fillForce2D(forceX, forceY, tmpX, tmpY)
#endif
 ...
 forceZ = forceZ + tmpZ
...
#else
     ! KAXIS
     kstr_F(LOW,LOW:HIGH) = 1;   kstr_F(HIGH,LOW:HIGH)= 1
     kstr_A(LOW,LOW:HIGH) = 1;   kstr_A(HIGH,LOW:HIGH)= 1
#endif
  ...
  call Timers_start('ib_updateVel') 

#ifdef USE_LOC_VAR

#ifdef NOT_COPY_TO_LOC
  do lb=1,count
     ...
     facexData(VELC_FACE_VAR,blkLimits(LOW,IAXIS):blkLimits(HIGH,IAXIS)+1, &
                             ...
                             blkLimits(LOW,KAXIS):blkLimits(HIGH,KAXIS)) + &  
     ib_dt*                                                                &
                      forceX(blkLimits(LOW,IAXIS):blkLimits(HIGH,IAXIS)+1, &
                             blkLimits(LOW,JAXIS):blkLimits(HIGH,JAXIS),   &
                             blkLimits(LOW,KAXIS):blkLimits(HIGH,KAXIS))
     ...
     faceyData(VELC_FACE_VAR,blkLimits(LOW,IAXIS):blkLimits(HIGH,IAXIS), &
                             ...
                             blkLimits(LOW,KAXIS):blkLimits(HIGH,KAXIS)) + &  
     ib_dt*                                                                &
                      forceY(blkLimits(LOW,IAXIS):blkLimits(HIGH,IAXIS), &
                             blkLimits(LOW,JAXIS):blkLimits(HIGH,JAXIS)+1,   &
                             blkLimits(LOW,KAXIS):blkLimits(HIGH,KAXIS))
     ...
     facezData(VELC_FACE_VAR,blkLimits(LOW,IAXIS):blkLimits(HIGH,IAXIS), &
                             ...
                             blkLimits(LOW,KAXIS):blkLimits(HIGH,KAXIS)+1) + &  
     ib_dt*                                                                &
                      forceZ(blkLimits(LOW,IAXIS):blkLimits(HIGH,IAXIS), &
                             blkLimits(LOW,JAXIS):blkLimits(HIGH,JAXIS),   &
                             blkLimits(LOW,KAXIS):blkLimits(HIGH,KAXIS)+1)
#endif
...
#endif
  enddo
#else /* For ifdef NOT_COPY_TO_LOC */

  call Timers_start('ib_copyToLoc')
  call sm_copyToLocal(VELC_FACE_VAR,tmpX,GRID_IHI_GC+1,GRID_JHI_GC,GRID_KHI_GC,NFACE_VARS,FACEX)
  call sm_copyToLocal(VELC_FACE_VAR,tmpY,GRID_IHI_GC,GRID_JHI_GC+1,GRID_KHI_GC,NFACE_VARS,FACEY)
#if NDIM == MDIM
  call sm_copyToLocal(VELC_FACE_VAR,tmpZ,GRID_IHI_GC,GRID_JHI_GC,GRID_KHI_GC+1,FACEZ)
#endif
  call Timers_stop('ib_copyToLoc')

  tmpX(GRID_ILO:GRID_IHI+1,GRID_JLO:GRID_JHI,GRID_KLO:GRID_KHI) =  &
  tmpX(GRID_ILO:GRID_IHI+1,GRID_JLO:GRID_JHI,GRID_KLO:GRID_KHI) +  &
forceX(GRID_ILO:GRID_IHI+1,GRID_JLO:GRID_JHI,GRID_KLO:GRID_KHI)*ib_dt

  tmpY(GRID_ILO:GRID_IHI,GRID_JLO:GRID_JHI+1,GRID_KLO:GRID_KHI) =  &
  tmpY(GRID_ILO:GRID_IHI,GRID_JLO:GRID_JHI+1,GRID_KLO:GRID_KHI) +  &
forceY(GRID_ILO:GRID_IHI,GRID_JLO:GRID_JHI+1,GRID_KLO:GRID_KHI)*ib_dt

#if NDIM == MDIM
  tmpZ(GRID_ILO:GRID_IHI,GRID_JLO:GRID_JHI,GRID_KLO:GRID_KHI+1) =  &
  tmpZ(GRID_ILO:GRID_IHI,GRID_JLO:GRID_JHI,GRID_KLO:GRID_KHI+1) +  &
forceZ(GRID_ILO:GRID_IHI,GRID_JLO:GRID_JHI,GRID_KLO:GRID_KHI+1)*ib_dt
#endif
  call Timers_start('ib_copyToGlo')
  call sm_copyToGlobal(VELC_FACE_VAR,tmpX,GRID_IHI_GC+1,GRID_JHI_GC,GRID_KHI_GC,FACEX)
  call sm_copyToGlobal(VELC_FACE_VAR,tmpY,GRID_IHI_GC,GRID_JHI_GC+1,GRID_KHI_GC,FACEY)
#if NDIM == MDIM
  call sm_copyToGlobal(VELC_FACE_VAR,tmpZ,GRID_IHI_GC,GRID_JHI_GC,GRID_KHI_GC+1,FACEZ)
#endif
  call Timers_stop('ib_copyToGlo')

#endif /* For ifdef NOT_COPY_TO_LOC */
...
end subroutine gr_sbUpdateForces_loc
