...
!!
!!***
subroutine ib_forcing_loc3D(ibd,p,blockID,particleData,fx,fy,fz)
...
#include "ImBound.h"
  ...
  real,INTENT(OUT) :: fx(GRID_IHI_GC+1,GRID_JHI_GC,GRID_KHI_GC)
  real,INTENT(OUT) :: fy(GRID_IHI_GC,GRID_JHI_GC+1,GRID_KHI_GC)
  real,INTENT(OUT) :: fz(GRID_IHI_GC,GRID_JHI_GC,GRID_KHI_GC+1)
  !! -----------------------------------------------------
  ...
  do i = 1,NDIM
     ...
     if (force_fl(i)) then
        ...
        call ib_extrapEpoints_loc3D(part_Pos,sb,hl,del,ib_ielem(:,:,i),ib_phile(:,:,i),   &        
                              ib_FuL(i),blockID,FACE_IND(i),fx,fy,fz)
  ...
  write(*,*) 'Routine ib_forcing_loc2D has not been tested for fixed body flog '
  stop
...
end subroutine ib_forcing_loc3D
