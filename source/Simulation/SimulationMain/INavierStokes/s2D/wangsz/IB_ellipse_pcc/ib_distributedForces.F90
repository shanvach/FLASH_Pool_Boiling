...
subroutine ib_distributedForces(blockID, particleData, vortx, vorty, vortz)
  ...
  real, dimension(MDIM) :: xbe_v, xbe_p
  real :: sp, ddpdxndxn
...
! To take account the variation of pressure in normal idrection on coarse mesh
  ...
  xbe_p(IAXIS) = xp + nxp*h*sp
  xbe_p(JAXIS) = yp + nyp*h*sp
#if NDIM == 3
  xbe_p(KAXIS) = zp + nzp*h*sp
#else
  xbe_p(KAXIS) = 0.
#endif
...
! Project external and marker Velocity on the plane:
  ...
  do jj = 1, MDIM
    do ii = 1, MDIM
      ddpdxndxn = ddpdxndxn +  np(ii)*np(jj)*ddp(ii,jj)
    enddo
  enddo  
  ...
  ddpdxndxn = 0.
!-------------
#ifdef TWO_POINTSP_NORMAL
  ! Compute dpdxt from other pressure values:
  ! External Point Position:
  normt = h/2.
  xbe2(IAXIS) = xp + nxp*h*sp + nxp*normt
  xbe2(JAXIS) = yp + nyp*h*sp + nyp*normt
#if NDIM == 3
  xbe2(KAXIS) = zp + nzp*h*sp + nzp*normt
#else
  xbe2(KAXIS) = 0.
#endif
  zpres2 = 0.
  ...
  !! Point 3:
  xbe3(IAXIS) = xp + nxp*h*sp - nxp*normt
  xbe3(JAXIS) = yp + nyp*h*sp - nyp*normt
#if NDIM == 3
  xbe3(KAXIS) = zp + nzp*h*sp - nzp*normt
#else
  xbe3(KAXIS) = 0.
#endif
  zpres3 = 0.
  ...
  !dpdxt = (zpres2-zpres)/normt ! This 1st order approx would save us from
                                ! computing zpres3
  dpdxn = (zpres2-zpres3)/(2.*normt) 
  !ddpdxndxn = (zpres2+zpres3-2*zpres)/(normt*normt)
  ddpdxndxn = 0.
#endif /* TWO POINT flag */

!--------------
  !zL2   = zpres - dpdxn*h
  !zL2   = zpres - dpdxn*h*sp
  !zL2   = zpres - dpdxn*h*sp + ddpdxndxn*h*sp*h*sp*0.5
  zL2   = zpres - dpdxn*h*sp*0.5
