...
!  logical :: gcMask(NUNK_VARS+NDIM*NFACE_VARS)
  ...
  !real :: tempx(NFACE_VARS, GRID_IHI_GC+1,GRID_JHI_GC  ,GRID_KHI_GC)
  !real :: tempy(NFACE_VARS, GRID_IHI_GC,GRID_JHI_GC+1  ,GRID_KHI_GC)
  real :: tempx(GRID_IHI_GC+1,GRID_JHI_GC  ,GRID_KHI_GC)
  real :: tempy(GRID_IHI_GC,GRID_JHI_GC+1  ,GRID_KHI_GC)

  !real, allocatable, dimension(:,:,:,:) :: tempAllocX, tempAllocY
  real, allocatable, dimension(:,:,:) :: tempAllocX, tempAllocY

  
  !allocate(tempAllocX(NFACE_VARS, GRID_IHI_GC+1,GRID_JHI_GC  ,GRID_KHI_GC))
  !allocate(tempAllocY(NFACE_VARS, GRID_IHI_GC,GRID_JHI_GC+1  ,GRID_KHI_GC))
  allocate(tempAllocX(GRID_IHI_GC+1,GRID_JHI_GC  ,GRID_KHI_GC))
  allocate(tempAllocY(GRID_IHI_GC,GRID_JHI_GC+1  ,GRID_KHI_GC))
...
! Turn off to reduce the time in printing on screen, Shizhao Wang, Jan 7, 2014
...
! Shizhao add #if NDIM == MDIM ... #endif.
#if NDIM == MDIM   
        facezData(FORC_FACE_VAR,:,:,:) = forcezData(:,:,:,blockID)
#endif

!        write(*,*) 'Size of facexData:', size(facexData,1),size(facexData,2),size(facexData,3), size(facexData,4), 'myID:', gr_meshMe
        ! Low Boundary:
