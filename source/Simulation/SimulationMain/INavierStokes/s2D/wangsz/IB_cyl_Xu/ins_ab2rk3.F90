...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  ! Shizhao Wang test the pressure correction for viscous flow
!  real, dimension(GRID_IHI_GC+4,GRID_JHI_GC+4,GRID_KHI_GC) :: pppxi
!  real, dimension(GRID_IHI_GC+4,GRID_JHI_GC+4,GRID_KHI_GC) :: pppyi
!  real, dimension(GRID_IHI_GC+1+4,GRID_JHI_GC+4,GRID_KHI_GC) :: pupxi
!  real, dimension(GRID_IHI_GC+4,GRID_JHI_GC+1+4,GRID_KHI_GC) :: pvpxi
!  real, dimension(GRID_IHI_GC+1+4,GRID_JHI_GC+4,GRID_KHI_GC) :: pupyi
!  real, dimension(GRID_IHI_GC+4,GRID_JHI_GC+1+4,GRID_KHI_GC) :: pvpyi
  real, dimension(GRID_IHI_GC,GRID_JHI_GC,GRID_KHI_GC) :: pppxi
  real, dimension(GRID_IHI_GC,GRID_JHI_GC,GRID_KHI_GC) :: pppyi
  real, dimension(GRID_IHI_GC+1,GRID_JHI_GC,GRID_KHI_GC) :: pupxi
  real, dimension(GRID_IHI_GC+1,GRID_JHI_GC,GRID_KHI_GC) :: pupyi
  real, dimension(GRID_IHI_GC,GRID_JHI_GC+1,GRID_KHI_GC) :: pvpxi
  real, dimension(GRID_IHI_GC,GRID_JHI_GC+1,GRID_KHI_GC) :: pvpyi
...
#ifdef SET_TG_FLOW
!##############################################################
  ! Shizhao Wang set the artificial flows
...
#if NDIM == 3
    print*, 'the routine ins_setArtifFlows does not work for 3D flows'
    stop
     ...
     pupxi = 0.
     pvpxi = 0.
     call ins_setArtifFlow(blockID, pupxi, pvpxi, pppxi,           &
                        blkLimits(LOW,IAXIS),blkLimits(HIGH,IAXIS),&
                        blkLimits(LOW,JAXIS),blkLimits(HIGH,JAXIS),&
                        del(DIR_X),del(DIR_Y))
 
     ! save gradient
     solnData(PRES_VAR,:,:,:)  = pppxi(:,:,:)
     facexData(VELC_FACE_VAR,:,:,:) = pupxi(:,:,:)
     faceyData(VELC_FACE_VAR,:,:,:) = pvpxi(:,:,:)
...
!##############################################################
  ! Shizhao Wang test the pressure correction for viscous force
...
#if NDIM == 3
    print*, 'the routine ins_getGrad2D does not work for 3D flows'
    stop

#elif NDIM ==2
!     print*, 'blkLimits', blkLimits
!     print*, 'blkLimitsGC', blkLimitsGC
!     print*, 'GRID_KHI_GC', GRID_IHI_GC, GRID_JHI_GC, GRID_KHI_GC
     ...
     pvpxi = 0.
     pvpyi = 0.
     call ins_getGrad2d(blockID, facexData(VELC_FACE_VAR,:,:,:),   &
                        faceyData(VELC_FACE_VAR,:,:,:),            &
                        solnData(PRES_VAR,:,:,:),                  &
                        blkLimits(LOW,IAXIS),blkLimits(HIGH,IAXIS),&
                        blkLimits(LOW,JAXIS),blkLimits(HIGH,JAXIS),&
     ...
     ! save gradient
     solnData(PPPX_VAR,:,:,:)  = pppxi(:,:,:)
     solnData(PPPY_VAR,:,:,:)  = pppyi(:,:,:)
     facexData(PPX_FACE_VAR,:,:,:) = pupxi(:,:,:)
     facexData(PPY_FACE_VAR,:,:,:) = pupyi(:,:,:)
     faceyData(PPX_FACE_VAR,:,:,:) = pvpxi(:,:,:)
     faceyData(PPY_FACE_VAR,:,:,:) = pvpyi(:,:,:)
