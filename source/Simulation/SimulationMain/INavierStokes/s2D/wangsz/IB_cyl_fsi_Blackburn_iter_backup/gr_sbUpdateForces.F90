...
!  write(*,*) 'Entrance of the routine:', gr_meshMe 
...
!  write(*,*) 'get the block list', gr_meshMe
...
!  write(*,*) 'Initialize the FORO', gr_meshMe

  call MPI_Barrier (gr_meshComm, ierr) 

  do b = 1, gr_sbNumBodies
    bodyInfo => gr_sbBodyInfo(b)
!    write(*,*) 'associated the bodyInfo', gr_meshMe
    if(gr_sbParticleCount(b) .gt. 0) then    
!  call MPI_Barrier (gr_meshComm, ierr) 
!    write(*,*) bodyInfo % myPE, bodyInfo % bodyMaster, gr_meshMe
!    write(*,*) 'ptc', gr_sbParticleCount(b), gr_meshMe
    bodyInfo % particles(FULO_PART_PROP,:) = 0. 
!    write(*,*) 'set the FULO', gr_meshMe
!  call MPI_Barrier (gr_meshComm, ierr) 
    bodyInfo % particles(FVLO_PART_PROP,:) = 0. 
!    write(*,*) 'set the FVLO', gr_meshMe
!  call MPI_Barrier (gr_meshComm, ierr) 
#if NDIM == MDIM
...
!  call MPI_Barrier (gr_meshComm, ierr) 
!  write(*,*) 'Initialize the FVEL0', gr_meshMe
...
!  call MPI_Barrier (gr_meshComm, ierr) 
!  write(*,*) 'Maximum Forcing itractions:', maxForceIter, gr_meshMe
!  call MPI_Barrier (gr_meshComm, ierr) 

  do iForceIter = 1, maxForceIter ! iterative forcing
...
! Fill guard cells
   ...
   call Timers_stop("Gcell_tempVelocs")
    ...
    write(indNstep, '(I6.6)') NStep/1
    call gr_intepVel('parts-uv-step-'//indNStep//'-iter',iForceIter)
    !call sm_uvTecplot('parts-uv-step-'//indNStep//'-iter-',iForceIter)
    endif
