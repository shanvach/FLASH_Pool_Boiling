...
subroutine sm_el01_mapParticles_inout(body, e, ptelem,  &
  ...
  use Driver_data, only: dr_simTime, dr_dtInit,dr_meshMe
  ...
  do a = 1,nen_e
     ...
     umax = -1.5*sin(2*pi*dr_simTime);
     if(umax == 0) umax = -1.5*sin(2*pi*dr_simTime+dr_dtInit);
     if(idx_in == 1) then
       ind = e+(a-1)
       ud(a,IAXIS) =  0.0
       !ud(a,JAXIS) = -1.5*(body%x(ind)*(1.0-body%x(ind)))*4.0 !Steady Parabolic Vel Profile
        ud(a,JAXIS) = umax*(body%x(ind)*(1.0-body%x(ind)))*4.0 !Oscillatory Vel Profile 
