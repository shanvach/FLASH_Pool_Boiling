...
subroutine sm_surf_MapParticles(ibd)
  ...
  real, allocatable, dimension(:) :: xacc,yacc,zacc,xnrm,ynrm,znrm, outloc
  ...
  do e=1,nel
     ...
     select case( sm_bodyInfo(ibd)%ws_eltype(e) )
        ...
        case( TWO_NODE_LINE )
           call sm_el01_mapParticles_inout(sm_bodyInfo(ibd), e, ptelem, &
     ...
     gr_sbBodyInfo(ibd) % particles(OUTLOC_PART_PROP,numPart+1:numPart+ptelem) = outloc(1:ptelem)
  ...
  deallocate( areai , loc_num, outloc )
#if NDIM == MDIM
