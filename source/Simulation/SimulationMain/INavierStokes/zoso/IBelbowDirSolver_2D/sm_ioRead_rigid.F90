...
!!$  enddo
  ...
  if (Body%MetaBody .gt. CONSTANT_ZERO) then
    ...
    dims = (/1,1,1/)
    CALL h5dopen_f (file, "WetSurface/nin", dset, h5err)
    call sm_io_checkHdfErr(h5err, 'WetSurface/nin')
    CALL h5dread_f(dset, H5T_NATIVE_INTEGER, Body%ws_in, dims, h5err)
    CALL h5dclose_f(dset , h5err)          

    dims = (/1,1,1/)
    CALL h5dopen_f (file, "WetSurface/nout", dset, h5err)
    call sm_io_checkHdfErr(h5err, 'WetSurface/nout')
    CALL h5dread_f(dset, H5T_NATIVE_INTEGER,Body%ws_out, dims, h5err)
    CALL h5dclose_f(dset , h5err)          
  
    ! ws_IEN_inflow:
    dims = (/sm_nen(Body%ws_max_eltype),Body%ws_in,1/)
    allocate(Body%ws_IEN_inflow(sm_nen(Body%ws_max_eltype),Body%ws_in))
    CALL h5dopen_f (file, "WetSurface/inflow", dset, h5err)
    call sm_io_checkHdfErr(h5err, 'WetSurface/inflow')
    CALL h5dread_f(dset, H5T_NATIVE_INTEGER, Body%ws_IEN_inflow, dims, h5err)
    CALL h5dclose_f(dset , h5err)        

    ! ws_IEN_outflow:
    dims = (/sm_nen(Body%ws_max_eltype),Body%ws_out,1/)
    allocate(Body%ws_IEN_outflow(sm_nen(Body%ws_max_eltype),Body%ws_out))
    CALL h5dopen_f (file, "WetSurface/outflow", dset, h5err)
    call sm_io_checkHdfErr(h5err, 'WetSurface/outflow')
    CALL h5dread_f(dset, H5T_NATIVE_INTEGER, Body%ws_IEN_outflow, dims, h5err)
    CALL h5dclose_f(dset , h5err)
