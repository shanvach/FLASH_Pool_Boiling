...
#include "Flash.h"
  ...
  use Plasma_data, ONLY : pls_prol_method
...
#include "IncompNS.h"
  ...
  if (beforeOrAfter == BEFORE) then
     ...
     if (pls_prol_method == INJECTION_PROL) then
        !! zeroth order
        interp_mask_facex(MAGF_FACE_VAR)=0
        interp_mask_facey(MAGF_FACE_VAR)=0
        interp_mask_facez(MAGF_FACE_VAR)=0

     elseif (pls_prol_method == LINEAR_PROL) then
        !! Linear
        interp_mask_facex(MAGF_FACE_VAR)=1
        interp_mask_facey(MAGF_FACE_VAR)=1
        interp_mask_facez(MAGF_FACE_VAR)=1

     elseif (pls_prol_method == QUADRATIC_PROL) then
        !! Quadratic
        interp_mask_facex(MAGF_FACE_VAR)=2
        interp_mask_facey(MAGF_FACE_VAR)=2
        interp_mask_facez(MAGF_FACE_VAR)=2

     elseif (pls_prol_method == DIVPRES_PROL) then

        ! Divergence preserving prolongation: 
        ins_divpres_fc_vars(1:totFaces,1) = MAGF_FACE_VAR

        !Divergence cleaning in block boundaries neighboring old blocks:
        i_divf_fc_vars(1:totFaces,1) = MAGF_FACE_VAR
