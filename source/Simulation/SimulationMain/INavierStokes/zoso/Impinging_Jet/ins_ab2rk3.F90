...
#include "SolidMechanics.h"
  ...
  use IncompNS_data, ONLY : ins_isgs, ins_invRe, ins_intschm, ins_prescoeff, ins_meshMe,&
                            ...
                            ins_meshNumProcs, ins_vardt, rkstep, ins_intschm_type
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  real :: ures, vres, uresall, vresall
...
!!$#endif
  ...
  if(blockCount .gt. 0) then
        ures = ures/blockCount
        vres = vres/blockCount
  end if
  ...
  call MPI_Allreduce(ures, uresall, 1, FLASH_REAL,&
                     MPI_SUM, MPI_COMM_WORLD, ierr)

  call MPI_Allreduce(vres, vresall, 1, FLASH_REAL,&
                     MPI_SUM, MPI_COMM_WORLD, ierr)

  vresall = vresall/ins_meshNumProcs
  uresall = uresall/ins_meshNumProcs
  ...
  if (ins_meshMe .eq. MASTER_PE) then
     ...
     write(*,'(A24,2g14.6)') ' ures, vres   =',sqrt(uresall),sqrt(vresall)
  endif
