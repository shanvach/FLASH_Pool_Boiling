...
subroutine Driver_computeDt(nbegin, nstep, &
  ...
  use Plasma_interface, ONLY: Plasma_computeDt
...
#include "Flash.h"
  ...
  integer, parameter :: HYDRO=1,BURN=2,GRAV=3,HEAT=4,COOL=5,TEMP=6,&
                        ...
                        RADTRANS=12,STS=13,INS=14,SOLID=15,PLS=16
  logical :: printToScrn
  real :: extraHydroInfo
...
!!$                           dtLocal(1,COSMO), lminloc(:,COSMO) )
  ...
  !print *,"dt_ins_main:", dtLocal(1,INS)
  ...
  ! Plasma:
  call Plasma_computeDt(dtLocal(1,PLS),lminloc(:,PLS))
