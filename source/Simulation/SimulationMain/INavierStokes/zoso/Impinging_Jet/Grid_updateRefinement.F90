...
!!$  use Grid_data, ONLY : gr_blkList, gr_eosMode
  ...
  use Plasma_data, ONLY : pls_predcorrflg,pls_outflowgridChanged
...
!!$  integer :: count
  ...
  if (mod(nstep, gr_nrefs) == 0) then
     ...
     interp_mask_facex_res(MAGF_FACE_VAR) = 1
     interp_mask_facey(MAGF_FACE_VAR) = 2
     interp_mask_facey_res(MAGF_FACE_VAR) = 1
     interp_mask_facez(MAGF_FACE_VAR) = 2
     interp_mask_facez_res(MAGF_FACE_VAR) = 1
