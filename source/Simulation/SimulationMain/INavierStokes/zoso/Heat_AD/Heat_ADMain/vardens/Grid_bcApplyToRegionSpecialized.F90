...
#include "Flash.h"
  ...
  if(gridDataStruct == CENTER .and. mask(TEMP_VAR)) then
     ...
     if (face == LOW) then 
         ...
         if (axis == IAXIS) then
              ...
              do i = 1,guard
                 ...
                 ! Adiabatic BC
                 regionData(i,1:je,1:ke,TEMP_VAR) = regionData(k-i,1:je,1:ke,TEMP_VAR)
                  
                 ! Dirichlet BC
              !   regionData(k-i,1:je,1:ke,TEMP_VAR) = 2*ht_Twall - regionData(i,1:je,1:ke,TEMP_VAR)
              ...
              do i = 1,guard
               ...
               !   regionData(i,1:je,1:ke,TEMP_VAR) = regionData(k-i,1:je,1:ke,TEMP_VAR)
                  ...
                  ! Dirichlet BC
                  regionData(i,1:je,1:ke,TEMP_VAR) = 2*ht_Twall_low - regionData(k-i,1:je,1:ke,TEMP_VAR)
              ...
              !        else
                       ! Adiabatic BC
              !         regionData(i,j,1:ke,TEMP_VAR) = regionData(k-i,j,1:ke,TEMP_VAR)
              !        end if
              !   end do
              !end do
         ...
         if (axis == IAXIS) then
              ...
              do i = 1,guard
                 ...
                 ! Adiabatic BC
                 regionData(k-i,1:je,1:ke,TEMP_VAR) = regionData(i,1:je,1:ke,TEMP_VAR)
              ...
              do i = 1,guard
                  ...
                  ! Dirichlet BC
                  regionData(k-i,1:je,1:ke,TEMP_VAR) = 2*ht_Twall_high - regionData(i,1:je,1:ke,TEMP_VAR)
             
                  ! Neumann BC 
              !    regionData(k-i,1:je,1:ke,TEMP_VAR) = (del(DIR_Y)*ht_Nu)*&
              !                                          (ht_Twall-regionData(i,1:je,1:ke,TEMP_VAR))&
              !                                          +regionData(i,1:je,1:ke,TEMP_VAR)
              ...
              !        if(xcell >=0.0 .and. xcell<=0.25) then
                       ! Dirichlet BC
              !         regionData(k-i,j,1:ke,TEMP_VAR) = 2*ht_Twall-regionData(i,j,1:ke,TEMP_VAR)
              !        else
                       ! Adiabatic BC
              !         regionData(k-i,j,1:ke,TEMP_VAR) = regionData(i,j,1:ke,TEMP_VAR)
              !        end if
              !   end do
              !end do 
