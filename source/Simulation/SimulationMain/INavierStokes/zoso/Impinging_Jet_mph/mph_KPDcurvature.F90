...
!=========================================================================
        ...
        subroutine mph_KPDcurvature2DAB(s,s2,crv,rho1x,rho2x,rho1y,rho2y,pf,pf2,w,sigx,sigy,dx,dy, &
...
#include "Flash.h"
        ...
        real, dimension(:,:,:), intent(inout):: s,s2,pf2,crv,rho1x,rho2x,rho1y, &
...
!        end do
        ...
        do j = jy1-1,jy2+1
           do i = ix1-1,ix2+1
              ...
              if(s2(i,j,k) .ge. 0.0) then
                 pf2(i,j,k) = 1.                       !- kpd - Set phase function on each side of interface
        ...
        do j = jy1-1,jy2+1
           do i = ix1-1,ix2+1
              a1 = (pf2(i-1,j,k) + pf2(i,j,k)) / 2.                       
              a2 = pf2(i-1,j,k)  /abs(pf2(i-1,j,k)  +eps) * &
                   pf2(i,j,k)/abs(pf2(i,j,k)+eps)
        ...
        do i = ix1-1,ix2+1
           do j = jy1-1,jy2+1
              a1 = (pf2(i,j-1,k) + pf2(i,j,k)) / 2.           
              a2 = pf2(i,j-1,k)  /abs(pf2(i,j-1,k)  +eps) * &
                   pf2(i,j,k)/abs(pf2(i,j,k)+eps)
