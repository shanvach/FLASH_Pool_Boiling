...
#include "Flash.h"
       ...
       do k = kz1-1,kz2+1
        do j = jy1-1,jy2+1
         do i = ix1-1,ix2+1
              ...
              if(s(i,j,k)*s(i,j,k+1) .le. 0.) mflg(i,j,k+1)   = 1.0
              if(s(i,j,k)*s(i-1,j,k) .le. 0.) mflg(i-1,j,k)   = 1.0
              if(s(i,j,k)*s(i,j-1,k) .le. 0.) mflg(i,j-1,k)   = 1.0
              if(s(i,j,k)*s(i,j,k-1) .le. 0.) mflg(i,j,k-1)   = 1.0
              ...
              if(s(i,j,k)*s(i,j,k+2) .le. 0.) mflg(i,j,k+2)   = 1.0
              if(s(i,j,k)*s(i-2,j,k) .le. 0.) mflg(i-2,j,k)   = 1.0
              if(s(i,j,k)*s(i,j-2,k) .le. 0.) mflg(i,j-2,k)   = 1.0
              if(s(i,j,k)*s(i,j,k-2) .le. 0.) mflg(i,j,k-2)   = 1.0
              ...
              if(s(i,j,k)*s(i,j,k+3) .le. 0.) mflg(i,j,k+3)   = 1.0
              if(s(i,j,k)*s(i-3,j,k) .le. 0.) mflg(i-3,j,k)   = 1.0
              if(s(i,j,k)*s(i,j-3,k) .le. 0.) mflg(i,j-3,k)   = 1.0
              if(s(i,j,k)*s(i,j,k-3) .le. 0.) mflg(i,j,k-3)   = 1.0

              if(s(i,j,k)*s(i+4,j,k) .le. 0.) mflg(i+4,j,k)   = 1.0
              if(s(i,j,k)*s(i,j+4,k) .le. 0.) mflg(i,j+4,k)   = 1.0
              if(s(i,j,k)*s(i,j,k+4) .le. 0.) mflg(i,j,k+4)   = 1.0
              if(s(i,j,k)*s(i-4,j,k) .le. 0.) mflg(i-4,j,k)   = 1.0
              if(s(i,j,k)*s(i,j-4,k) .le. 0.) mflg(i,j-4,k)   = 1.0
              if(s(i,j,k)*s(i,j,k-4) .le. 0.) mflg(i,j,k-4)   = 1.0

              if(s(i,j,k)*s(i+5,j,k) .le. 0.) mflg(i+5,j,k)   = 1.0
              if(s(i,j,k)*s(i,j+5,k) .le. 0.) mflg(i,j+5,k)   = 1.0
              if(s(i,j,k)*s(i,j,k+5) .le. 0.) mflg(i,j,k+5)   = 1.0
              if(s(i,j,k)*s(i-5,j,k) .le. 0.) mflg(i-5,j,k)   = 1.0
              if(s(i,j,k)*s(i,j-5,k) .le. 0.) mflg(i,j-5,k)   = 1.0
              if(s(i,j,k)*s(i,j,k-5) .le. 0.) mflg(i,j,k-5)   = 1.0
       ...
       nrmx(ix1:ix2,jy1:jy2,kz1:kz2) =           &
           (( s(ix1+1:ix2+1,jy1:jy2,kz1:kz2) -   &
             ...
             + ((s(ix1:ix2,jy1:jy2,kz1+1:kz2+1) - &
             s(ix1:ix2,jy1:jy2,kz1-1:kz2-1))/2./dz)**2  )

        nrmy(ix1:ix2,jy1:jy2,kz1:kz2) =           &
           (( s(ix1:ix2,jy1+1:jy2+1,kz1:kz2) -   &
        ...
        nrmz(ix1:ix2,jy1:jy2,kz1:kz2) =           &
           (( s(ix1:ix2,jy1:jy2,kz1+1:kz2+1) -   &
             s(ix1:ix2,jy1:jy2,kz1-1:kz2-1) )/2./dz)/ &
             sqrt( ((s(ix1+1:ix2+1,jy1:jy2,kz1:kz2) - &
             s(ix1-1:ix2-1,jy1:jy2,kz1:kz2))/2./dx)**2 &
             + ((s(ix1:ix2,jy1+1:jy2+1,kz1:kz2) - &
             s(ix1:ix2,jy1-1:jy2-1,kz1:kz2))/2./dy)**2 &
...
!=========================================================================
        ...
        use Multiphase_data, ONLY : mph_baseCount, mph_baseRadius, mph_baseCountAll
...
#include "constants.h"
        ...
        fmic = (ht_fmic*mph_baseRadius)/(mph_baseCountAll)
           ...
           do j = jy1-1,jy2
              ...
              if(pf(i,j,k).eq.0..and.pf(i+1,j,k).eq.1.) then
                 ...
                 if(ycell==0.5*del(JAXIS)) then
                        ...
                        sigx(i+1,j,k) = sigx(i+1,j,k) - fmic/aa/dx
              ...
              if(pf(i,j,k).eq.0..and.pf(i,j+1,k).eq.1.) then
                 ...
                 if(ycell==0.5*del(JAXIS)) then
                        ...
                        sigy(i,j+1,k) = sigy(i,j+1,k) - fmic/aa/dy
              ...
              if(pf(i,j,k).eq.0..and.pf(i,j,k+1).eq.1.) then
                 ...
                 if(ycell==0.5*del(JAXIS)) then 
                        ...
                        sigz(i,j,k+1) = sigz(i,j,k+1) - fmic/aa/dz
