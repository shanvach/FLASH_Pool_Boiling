...
#include "IncompNS.h"
  ...
  real bsize(MDIM),coord(MDIM),xCoord(GRID_IHI+1),yCoord(GRID_JHI+1)
  
  real del(MDIM),xcell,ycell, ycellp,zcell,rc,xcellp,zcellp, dxc,dyc
  ...
  real :: sumx,sumy,sumxAll,sumyAll,flg,centerx,centery
           ...
           do j=blkLimits(LOW,JAXIS),blkLimits(HIGH,JAXIS)
              do k=blkLimits(LOW,KAXIS),blkLimits(HIGH,KAXIS)
                 if (solnData(DFUN_VAR,i,j,k) .gt. 0) then
                   volSum = volSum + (1/(dx(i,CENTER,blockID)*dy(j,CENTER,blockID)*dz(k,CENTER,blockID)))
                 else
                   volSum_liq = volSum_liq +(1/(dx(i,CENTER,blockID)*dy(j,CENTER,blockID)*dz(k,CENTER,blockID)))
                 end if
              end do
           end do
        end do
        ...
        do i=blkLimits(LOW,IAXIS),blkLimits(HIGH,IAXIS)
           do j=blkLimits(LOW,JAXIS),blkLimits(HIGH,JAXIS)
              if (solnData(DFUN_VAR,i,j,1) .gt. 0) then
                volSum = volSum + (1/(dx(i,CENTER,blockID)*dy(j,CENTER,blockID)))
              else
                volSum_liq = volSum_liq + (1/(dx(i,CENTER,blockID)*dy(j,CENTER,blockID)))
              end if
           end do
        end do
        endif
...
!***********************************************************************************************
     ...
     do lb = 1,blockCount
        ...
        call Grid_getCellCoords(JAXIS,lb,FACES,.false.,yCoord,NYB+1)
        call Grid_getCellCoords(IAXIS,lb,FACES,.false.,xCoord,NXB+1)

        ! Point to blocks center and face vars:
        call Grid_getBlkPtr(blockID,solnData,CENTER)

       do k=1,1!blkLimitsGC(HIGH,KAXIS)
           do j=blkLimits(LOW,JAXIS),blkLimits(HIGH,JAXIS)
              do i=blkLimits(LOW,IAXIS),blkLimits(HIGH,IAXIS)

                 dxc = 1.0/dx(i,CENTER,blockID)
                 dyc = 1.0/dy(j,CENTER,blockID)      
                 xcell = xCoord(i-NGUARD)+dxc*0.5
                 ycell = yCoord(j-NGUARD)+dyc*0.5

                 !flg = ((solnData(DFUN_VAR,i,j,1)/abs(solnData(DFUN_VAR,i,j,1)))+1)*0.5

                 if(solnData(DFUN_VAR,i,j,1) .gt. 0) then
                    sumx = sumx + mph_rho1*xcell*dxc*dyc
                    sumy = sumy + mph_rho1*ycell*dxc*dyc
                 endif
     ...
     call MPI_Allreduce(sumx, sumxAll, 1, FLASH_REAL,&
                        MPI_SUM, MPI_COMM_WORLD, ierr)


    call MPI_Allreduce(sumy, sumyAll, 1, FLASH_REAL,&
                       MPI_SUM, MPI_COMM_WORLD, ierr)

    centerx = sumxAll/(volSumAll*mph_rho1)
    centery = sumyAll/(volSumAll*mph_rho1)

    if (mph_meshMe .eq. 0) print*,"----------------------------------------"
    if (mph_meshMe .eq. 0) print*,"Center of Mass: ",centerx,centery
    if (mph_meshMe .eq. 0) print*,"----------------------------------------"

   if (ins_meshMe .eq. 0) then
     open(1, file = 'centerOfMass.txt', action = 'write', access = 'append') !status ='replace'
         write(1,*) dr_simTime,centerx,centery
     close(1)
   endif
