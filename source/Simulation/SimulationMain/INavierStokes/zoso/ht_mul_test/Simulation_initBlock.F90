...
#include "Flash.h"
  ...
  real :: xcell,xedge,ycell,yedge,solnX
...
!          solnData(DFUN_VAR,i,j,k) = ycell - A0/(cosh(sqrt(3.0*A0)*xcell/2)**2)
          ...
          ! Circle
           ...
           !solnData(DFUN_VAR,i,j,k) = 0.001d0*(1.0d0 - sqrt((xcell-x0)**2+(ycell-y0)**2)/r0)

           !solnData(DFUN_VAR,i,j,k) = 0.00098 - ycell

           if (r0 - sqrt((xcell-x0)**2+(ycell-y0)**2) .ge. 0.) then

             solnData(TEMP_VAR,i,j,k) = 383.15

           else
             
             solnData(TEMP_VAR,i,j,k) = 373.15

           end if

           ! Akash - Stefan Problem
           !solnData(DFUN_VAR,i,j,k) = 1.0d0 - (0.5/ycell)
           !solnData(DFUN_VAR,i,j,k)  = 1.0d0 - (0.00025/(2*ycell))

           !solnX = .001-(2*0.066916063714766546815307639566313*&
           !            sqrt(timeEndAdv*((mph_thco1)/(mph_cp1))))

           !solnData(DFUN_VAR,i,j,k) = ycell - 0.0009
        enddo
     enddo
  enddo
  ...
  do k=1,1
     do j=1,blkLimitsGC(HIGH,JAXIS)
        do i=1,blkLimitsGC(HIGH,IAXIS)+1
          ...
          if (ycell .LE. 0.0) then
             facexData(VELC_FACE_VAR,i,j,k) = 1.0
          else
