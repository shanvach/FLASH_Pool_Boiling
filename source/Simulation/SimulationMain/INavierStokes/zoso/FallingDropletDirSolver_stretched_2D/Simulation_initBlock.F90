...
#include "Flash.h"
  ...
  real :: th_radii, radius, centerx,centery
...
!                   real(i - NGUARD - 1)/dx(CENTER,1,blockID) +   &
!                   0.5/dx(CENTER,1,blockID)

!           ycell = coord(JAXIS) - bsize(JAXIS)/2.0 +  &
!                   real(j - NGUARD - 1)/dy(CENTER,1,blockID)  +  &
!                   0.5/dy(CENTER,1,blockID)



           solnData(DFUN_VAR,i,j,k) = (sqrt((xcell-centerx)**2+(ycell-centery)**2+zcell**2)-radius)
