        ...
        if (axis == IAXIS) then 
          if (ivar == TEMP_VAR) then
            ...
            case (NEUMANN_HT)
              k = 2*guard+1
              do i = 1,guard
                regionData(i,1:je,1:ke,ivar) = del(IAXIS)*ht_Txl_value + regionData(k-i,1:je,1:ke,ivar)
              end do
              applied = .true.
          ...
          if (ivar == TEMP_VAR) then
            ...
            case (NEUMANN_HT)
              k = 2*guard+1
              do i = 1,guard
                regionData(i,1:je,1:ke,ivar) = del(JAXIS)*ht_Tyl_value + regionData(k-i,1:je,1:ke,ivar)
              end do
              applied = .true.
          ...
          if (ivar == TEMP_VAR) then
            ...
            case (DIRICHLET_HT)
             ...
             case (NEUMANN_HT)
               k = 2*guard+1
               do i = 1,guard
                 regionData(i,1:je,1:ke,ivar) = del(KAXIS)*ht_Tzl_value + regionData(k-i,1:je,1:ke,ivar)
               end do
               applied = .true.               
