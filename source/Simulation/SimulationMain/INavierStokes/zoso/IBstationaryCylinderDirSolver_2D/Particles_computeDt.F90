...
subroutine Particles_computeDt (blockID, dt_part, dt_minloc)
  ...
  use Grid_interface, ONLY : Grid_getBlkPhysicalSize, &
    Grid_getBlkBoundBox, Grid_getDeltas,Grid_getBlkIndexLimits,Grid_getCellMetrics
...
#include "constants.h"
  ...
  call Grid_getCellMetrics(IAXIS,blockID,LEFT_EDGE, .true.,dx_t(:,LEFT_EDGE,blockID), blkLimitsGC(HIGH,IAXIS)) 
  call Grid_getCellMetrics(IAXIS,blockID,CENTER,    .true.,dx_t(:,CENTER,blockID),    blkLimitsGC(HIGH,IAXIS)) 
  call Grid_getCellMetrics(IAXIS,blockID,RIGHT_EDGE,.true.,dx_t(:,RIGHT_EDGE,blockID),blkLimitsGC(HIGH,IAXIS)) 

  call Grid_getCellMetrics(JAXIS,blockID,LEFT_EDGE, .true.,dy_t(:,LEFT_EDGE,blockID), blkLimitsGC(HIGH,JAXIS)) 
  call Grid_getCellMetrics(JAXIS,blockID,CENTER,    .true.,dy_t(:,CENTER,blockID),    blkLimitsGC(HIGH,JAXIS)) 
  call Grid_getCellMetrics(JAXIS,blockID,RIGHT_EDGE,.true.,dy_t(:,RIGHT_EDGE,blockID),blkLimitsGC(HIGH,JAXIS))

#if NDIM ==3
  call Grid_getCellMetrics(KAXIS,blockID,LEFT_EDGE, .true.,dz_t(:,LEFT_EDGE,blockID),blkLimitsGC(HIGH,KAXIS)) 
  call Grid_getCellMetrics(KAXIS,blockID,CENTER,    .true.,dz_t(:,CENTER,blockID),   blkLimitsGC(HIGH,KAXIS)) 
  call Grid_getCellMetrics(KAXIS,blockID,RIGHT_EDGE,.true.,dz_t(:,RIGHT_EDGE,blockID),blkLimitsGC(HIGH,KAXIS)) 
#endif  

  delta(IAXIS) = 1.0/dx_t(1,CENTER,blockID)
  delta(JAXIS) = 1.0/dy_t(1,CENTER,blockID)
#if NDIM == MDIM
  delta(KAXIS) = 1.0/dz_t(1,CENTER,blockID)
#endif
