...
#include "Flash.h"
  ...
  real, dimension(GRID_IHI_GC,1,1) :: dx
  real, dimension(GRID_JHI_GC,1,1) :: dy
  real, dimension(GRID_KHI_GC,1,1) :: dz
     ...
     do j=1,blkLimitsGC(HIGH,JAXIS)
        do i=1,blkLimits(LOW,IAXIS)-1
           ...
           xcell = coord(IAXIS) - bsize(IAXIS)/2.0 +   &
                   ...
                   0.5/dx(i,CENTER,blockID) - u*t

           ycell  = coord(JAXIS) - bsize(JAXIS)/2.0 +  &
                   real(j - NGUARD - 1)/dy(j,CENTER,blockID)  +  &
                   0.5/dy(j,CENTER,blockID)
