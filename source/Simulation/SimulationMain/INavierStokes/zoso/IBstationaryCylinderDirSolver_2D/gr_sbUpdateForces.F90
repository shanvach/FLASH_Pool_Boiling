...
#include "Flash_mpi.h"
  ...
  real, dimension(GRID_IHI_GC,3,1) :: dx_t
  real, dimension(GRID_JHI_GC,3,1) :: dy_t
  real, dimension(GRID_KHI_GC,3,1) :: dz_t
...
! Fill guard cells
  ...
  do lb = 1,count
     ...
     !call Grid_getCellMetrics(IAXIS,blockID,LEFT_EDGE, .true.,dx_t(:,LEFT_EDGE,blockID), GRID_IHI_GC)!blkLimitsGC(HIGH,IAXIS)) 
     !call Grid_getCellMetrics(IAXIS,blockID,CENTER, .true.,dx_t(:,CENTER,blockID),    GRID_IHI_GC)!blkLimitsGC(HIGH,IAXIS)) 
     !call Grid_getCellMetrics(IAXIS,blockID,RIGHT_EDGE,.true.,dx_t(:,RIGHT_EDGE,blockID),GRID_IHI_GC)!blkLimitsGC(HIGH,IAXIS)) 

     !call Grid_getCellMetrics(JAXIS,blockID,LEFT_EDGE, .true.,dy_t(:,LEFT_EDGE,blockID), GRID_JHI_GC)!blkLimitsGC(HIGH,JAXIS)) 
     !call Grid_getCellMetrics(JAXIS,blockID,CENTER, .true.,dy_t(:,CENTER,blockID),    GRID_JHI_GC)!blkLimitsGC(HIGH,JAXIS)) 
     !call Grid_getCellMetrics(JAXIS,blockID,RIGHT_EDGE,.true.,dy_t(:,RIGHT_EDGE,blockID),GRID_JHI_GC)!blkLimitsGC(HIGH,JAXIS))

!#if NDIM ==3
     !call Grid_getCellMetrics(KAXIS,blockID,LEFT_EDGE, .true.,dz_t(:,LEFT_EDGE,blockID),GRID_KHI_GC)!blkLimitsGC(HIGH,KAXIS)) 
     !call Grid_getCellMetrics(KAXIS,blockID,CENTER, .true.,dz_t(:,CENTER,blockID),   GRID_KHI_GC)!blkLimitsGC(HIGH,KAXIS)) 
     !call Grid_getCellMetrics(KAXIS,blockID,RIGHT_EDGE,.true.,dz_t(:,RIGHT_EDGE,blockID),GRID_KHI_GC)!blkLimitsGC(HIGH,KAXIS)) 
!#endif  

     !dx = 1.0/dx_t(1,CENTER,blockID)
     !dy = 1.0/dy_t(1,CENTER,blockID)
!#if NDIM == MDIM
     !dz = 1.0/dz_t(1,CENTER,blockID)
!#endif
