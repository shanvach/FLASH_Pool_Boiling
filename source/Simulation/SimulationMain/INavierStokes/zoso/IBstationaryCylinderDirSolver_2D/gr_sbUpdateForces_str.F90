...
subroutine gr_sbUpdateForces
  ...
  use Grid_interface, ONLY : Grid_updateSolidBodyForces, Grid_getBlkPtr,     &
                             ...
                             Grid_fillGuardCells,Grid_getCellMetrics, &
                             Grid_getCellCoords
  ...
  use ImBound_data, only : ib_vel_flg, ib_dfun_flg, ib_dt, ib_maxIterForcing
...
#include "Flash_mpi.h"
  ...
  real, pointer, dimension(:,:,:,:) :: solnData,facexData,faceyData,facezData,facexData2,faceyData2,facezData2
...
!  logical :: gcMask(NUNK_VARS+NDIM*NFACE_VARS)
  ...
  real :: xCoord(GRID_IHI_GC), xFace(GRID_IHI_GC+1), yCoord(GRID_JHI_GC), yFace(GRID_JHI_GC+1)
  real :: zCoord(GRID_KHI_GC), zFace(GRID_KHI_GC+1)
...
!  maxForceIter = 1
  ...
  do b = 1, gr_sbNumBodies
     ...
     if (bodyInfo % myPE == bodyInfo % bodyMaster) then
...
#ifdef FLASH_GRID_PARAMESH
        if(ib_vel_flg) call gr_ptMove(particles2,NPART_PROPS, localCount,globalCount, moveDone)
#else
        pt_posinitialized=.true.

        if(ib_vel_flg) call Grid_moveParticles(particles2,NPART_PROPS, globalCount, localCount, &
        ...
        !print*,pt_indexCount,localCount,globalCount
        if(ib_vel_flg) call Grid_moveParticles(particles2,NPART_PROPS, globalCount, localCount, &
             pt_indexList, pt_indexCount, coords_in_blks) 
#endif
     endif
...
#ifdef ITER_FORCING
! Fill guard cells
   call Timers_start("Gcell_tempVelocs")
   ...
   call ins_fluxfix(NGUARD,nxc,nyc,nzc,nxc-1,nyc-1,nzc-1,&
                    count,listofBlocks)
#endif
   call Timers_stop("Gcell_tempVelocs")
  ...
  do lb = 1,count
     ...
     call Grid_getCellMetrics(IAXIS,blockID,LEFT_EDGE, .true.,dx(:,LEFT_EDGE,blockID), blkLimitsGC(HIGH,IAXIS)) 
     call Grid_getCellMetrics(IAXIS,blockID,CENTER,    .true.,dx(:,CENTER,blockID),    blkLimitsGC(HIGH,IAXIS)) 
     call Grid_getCellMetrics(IAXIS,blockID,RIGHT_EDGE,.true.,dx(:,RIGHT_EDGE,blockID),blkLimitsGC(HIGH,IAXIS)) 

    call Grid_getCellMetrics(JAXIS,blockID,LEFT_EDGE, .true.,dy(:,LEFT_EDGE,blockID), blkLimitsGC(HIGH,JAXIS)) 
    call Grid_getCellMetrics(JAXIS,blockID,CENTER,    .true.,dy(:,CENTER,blockID),    blkLimitsGC(HIGH,JAXIS)) 
    call Grid_getCellMetrics(JAXIS,blockID,RIGHT_EDGE,.true.,dy(:,RIGHT_EDGE,blockID),blkLimitsGC(HIGH,JAXIS)) 

#if NDIM ==3
    call Grid_getCellMetrics(KAXIS,blockID,LEFT_EDGE, .true.,dz(:,LEFT_EDGE,blockID),blkLimitsGC(HIGH,KAXIS)) 
    call Grid_getCellMetrics(KAXIS,blockID,CENTER,    .true.,dz(:,CENTER,blockID),   blkLimitsGC(HIGH,KAXIS)) 
    call Grid_getCellMetrics(KAXIS,blockID,RIGHT_EDGE,.true.,dz(:,RIGHT_EDGE,blockID),blkLimitsGC(HIGH,KAXIS)) 
#endif  

    call Grid_getCellCoords(IAXIS,blockID,CENTER,.true.,xCoord,blkLimitsGC(HIGH,IAXIS))
    call Grid_getCellCoords(JAXIS,blockID,CENTER,.true.,yCoord,blkLimitsGC(HIGH,JAXIS))
    call Grid_getCellCoords(KAXIS,blockID,CENTER,.true.,zCoord,blkLimitsGC(HIGH,KAXIS)) 
    call Grid_getCellCoords(IAXIS,blockID,FACES,.true.,xFace,blkLimitsGC(HIGH,IAXIS)+1)
    call Grid_getCellCoords(JAXIS,blockID,FACES,.true.,yFace,blkLimitsGC(HIGH,JAXIS)+1)
    call Grid_getCellCoords(KAXIS,blockID,FACES,.true.,zFace,blkLimitsGC(HIGH,KAXIS)+1)
     ...
     do i=blkLimits(LOW,IAXIS),blkLimits(HIGH,IAXIS)
      do j=blkLimits(LOW,JAXIS),blkLimits(HIGH,JAXIS)
       do k=blkLimits(LOW,KAXIS),blkLimits(HIGH,KAXIS)
          ...
          dxdydz = 1.0/(dx(i,LEFT_EDGE,blockID)*dy(i,CENTER,blockID)*dz(k,CENTER,blockID))
          ! X - Direction
          Fx_tmp = Fx_tmp + facexData(FORC_FACE_VAR,i,j,k)*dxdydz

          dxdydz = 1.0/(dx(i,CENTER,blockID)*dy(i,LEFT_EDGE,blockID)*dz(k,CENTER,blockID))
          ! Y - Direction
          Fy_tmp = Fy_tmp + faceyData(FORC_FACE_VAR,i,j,k)*dxdydz

#if NDIM == MDIM
          dxdydz = 1.0/(dx(i,CENTER,blockID)*dy(i,CENTER,blockID)*dz(k,LEFT_EDGE,blockID))
          ! Z - Direction
          Fz_tmp = Fz_tmp + facezData(FORC_FACE_VAR,i,j,k)*dxdydz
#endif
     ...
     do k=blkLimits(LOW,KAXIS),blkLimits(HIGH,KAXIS)
        ...
        do j=blkLimits(LOW,JAXIS),blkLimits(HIGH,JAXIS)
           ...
           do i=blkLimits(LOW,IAXIS),blkLimits(HIGH,IAXIS)
              ...
              MomzL = dxdydz*faceyData(FORC_FACE_VAR,i,j,k)*(xcell-0.) + &
                      MomzL
              ...
              MomxL = dxdydz*facezData(FORC_FACE_VAR,i,j,k)*(ycell-0.) + &
                      MomxL 
              ...
              MomyL = dxdydz*facexData(FORC_FACE_VAR,i,j,k)*(zcell-0.) + &
                      MomyL 
#endif
