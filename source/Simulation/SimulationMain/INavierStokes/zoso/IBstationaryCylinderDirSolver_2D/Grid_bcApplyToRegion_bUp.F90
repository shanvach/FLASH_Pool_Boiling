        ...
        if(face==LOW) then
           ...
           case(INFLOW_INS)
              ...
              if (isFace) then ! Set to zero velocities normal to boundary, up to boundary
                 ...
                 kb = endPoints(HIGH,KAXIS)
                 !do i=ja,jb
                    !if(mph_jet_vel(i,1,blockHandle).ne.mph_jet_vel(i,1,blockHandle))
                 !   print*,i,mph_jet_vel(i,1,blockHandle)
                 !enddo
                 !stop 
                 do i = 1,guard+1
...
!!              call Driver_abortFlash("unsupported boundary condition on Lower Face")
           ...
           case(OUTFLOW_INS) ! Convective outflow boundary condition.
              ...
              call Grid_getCellMetrics(IAXIS,blockHandle,CENTER,    .true.,dx(:,CENTER,blockHandle),    GRID_IHI_GC) 
              call Grid_getCellMetrics(IAXIS,blockHandle,RIGHT_EDGE,.true.,dx(:,RIGHT_EDGE,blockHandle),GRID_IHI_GC) 

              call Grid_getCellMetrics(JAXIS,blockHandle,CENTER,    .true.,dy(:,CENTER,blockHandle),    GRID_JHI_GC) 
              call Grid_getCellMetrics(JAXIS,blockHandle,RIGHT_EDGE,.true.,dy(:,RIGHT_EDGE,blockHandle),GRID_JHI_GC) 

              call Grid_getCellMetrics(KAXIS,blockHandle,CENTER,    .true.,dz(:,CENTER,blockHandle),    GRID_KHI_GC) 
              call Grid_getCellMetrics(KAXIS,blockHandle,RIGHT_EDGE,.true.,dz(:,RIGHT_EDGE,blockHandle),GRID_KHI_GC)
              ...
              if (isFace) then 
                 if(ivar == VELC_FACE_VAR) then ! U velocities on X face grid
                   ...
                   regionData(guard+1,1:je,1:ke,ivar) = uvel_x(guard+1,ja:jb,ka:kb,HIGH,blockHandle) -&
                    (ins_convvel(HIGH,axis)*alfadt*dx(NXB+guard,CENTER,blockHandle))* &
              ...
              elseif(gridDataStruct .eq. FACEY) then ! V velocities on X direction             
                 if(ivar == VELC_FACE_VAR) then                               
                   ...
                   vvel_x(guard+1,ja:jb,ka:kb,HIGH,blockHandle) = vvel_x(guard,ja:jb,ka:kb,HIGH,blockHandle) - & 
                        (ins_convvel(HIGH,axis)*alfadt*dx(NXB+guard,RIGHT_EDGE,blockHandle))* &
              ...
              if (isFace) then 
                 ...
                 do i = 1,guard+1
                   regionData(k-i,1:je,1:ke,ivar) = vvel_y(guard+1,ia:ib,ka:kb,HIGH,blockHandle) -&
                    (ins_convvel(HIGH,axis)*alfadt*dy(NYB+guard,CENTER,blockHandle))* &
              ...
              elseif(gridDataStruct .eq. FACEX) then ! U velocities on Y direction             
                 if(ivar == VELC_FACE_VAR) then                               
                   ...
                   uvel_y(guard+1,ia:ib,ka:kb,HIGH,blockHandle) = uvel_y(guard,ia:ib,ka:kb,HIGH,blockHandle) - & 
                        (ins_convvel(HIGH,axis)*alfadt*dy(NYB+guard,RIGHT_EDGE,blockHandle))* &
              ...
              if (isFace) then 
                 if(ivar == VELC_FACE_VAR) then ! W velocities on Z face grid
                   ...
                   regionData(guard+1,1:je,1:ke,ivar) = wvel_z(guard+1,ia:ib,ja:jb,HIGH,blockHandle) -&
                    (ins_convvel(HIGH,axis)*alfadt*dz(NZB+guard,CENTER,blockHandle))* &
              ...
              elseif(gridDataStruct .eq. FACEX) then ! U velocities on Z direction             
                 if(ivar == VELC_FACE_VAR) then                               
                   ...
                   uvel_z(guard+1,ia:ib,ja:jb,HIGH,blockHandle) = uvel_z(guard,ia:ib,ja:jb,HIGH,blockHandle) - & 
                        (ins_convvel(HIGH,axis)*alfadt*dz(NZB+guard,RIGHT_EDGE,blockHandle))* &
