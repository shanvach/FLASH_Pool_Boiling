...
!=============================================================================
  ...
  ! ONLY for HIGH x,y,z.
  if (any(gr_domainBC(HIGH,1:NDIM) .eq. NEUMANN_INS)) then    
...
!print*,"Conv A",blockID,blkLimitsGC(LOW,JAXIS),blkLimitsGC(HIGH,JAXIS),NGUARD,nxc
      ...
      ! High Y Boundary:
      if ((faces(HIGH,JAXIS) .eq. NEUMANN_INS)) then
        ! redistribute velocities to guardcells:
!print*,"Conv B",blockID
        ...
        do k=blkLimits(LOW,KAXIS),blkLimits(HIGH,KAXIS)
           do i=blkLimits(LOW,IAXIS),blkLimits(HIGH,IAXIS)
              ...
              convveli(HIGH,JAXIS) = max(convveli(HIGH,JAXIS),(faceyData(VELC_FACE_VAR,i,nyc,k)))
