...
#include "Flash.h"
  ...
  real, dimension(NXB+1,NYB+1,NZB+1) :: tpu,tpv,tpw,tpp,&
            ...
            tempp,mdotp,tnlp,tnvp,nxp,nyp,nzp,tprds,tmic,tpsi
...
!        count, mype
  ...
  i = TecIni('AMR3D'//NULLCHR,                            &
             'x y z u v w p wx wy wz Q div Phi T mdot Tnl Tnv Tmic psi nx ny nz Hflux'//NULLCHR,  &
             filename//NULLCHR,                           &
             './IOData/'//NULLCHR,                  &
             Debug,VIsdouble)
...
!                             unk(3,:,:,:,lb),divppstar)
     ...
     call centervals2corners(NGUARD,NXB,NYB,NZB,nxc,nyc,nzc,&
                             solnData(TMIC_VAR,:,:,:),tmic)

     call centervals2corners(NGUARD,NXB,NYB,NZB,nxc,nyc,nzc,&
                             solnData(PTES_VAR,:,:,:),tpsi)
