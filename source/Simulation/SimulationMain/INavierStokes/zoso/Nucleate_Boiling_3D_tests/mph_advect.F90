...
#include "IncompNS.h"
  ...
  integer :: psi_it, psi_count

  real    :: psi_xmins, psi_xplus, psi_zmins, psi_zplus, &
            psi_nxconv, psi_nzconv, psi_nxplus, psi_nxmins, psi_nzplus, psi_nzmins, psi_dt
     ...
     ! Point to blocks center and face vars:
        ...
        call Grid_getBlkPtr(blockID,facezData,FACEZ)
         ...
         do i=blkLimitsGC(LOW,IAXIS),blkLimitsGC(HIGH,IAXIS)
            ...
            if(abs(solnData(DFUN_VAR,i,NGUARD+1,k)) .le. 2.0*del(IAXIS)) then
            ...
            solnData(PTES_VAR,i,NGUARD+1,k) = mph_psi(i,k,blockID)
    ...
    do k=blkLimits(LOW,KAXIS),blkLimits(HIGH,KAXIS)
     do j=blkLimits(LOW,JAXIS),blkLimits(HIGH,JAXIS)
      do i=blkLimits(LOW,IAXIS),blkLimits(HIGH,IAXIS)
         ...
         nuc_dfun  = 0.15 - sqrt((xcell-sim_nuc_site_x(nuc_index))**2+(ycell-sim_nuc_site_y(nuc_index))**2+(zcell-sim_nuc_site_z(nuc_index))**2)
