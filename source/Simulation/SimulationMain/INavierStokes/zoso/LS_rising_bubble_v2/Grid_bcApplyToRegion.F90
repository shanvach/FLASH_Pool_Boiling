...
!*******************************************************************
              ...
              if (isFace) then ! Set to zero velocities normal to boundary, up to boundary
                 !------------------------------------------------------------------------------------------
                 if(ivar == VELC_FACE_VAR .OR. ivar == VELI_FACE_VAR .OR. ivar == VELI_FACE_VAR ) then
                  !-- KPD Original --
#ifdef TWO_PHASE_SEP
                 do i = 1,guard+1
...
!!              call Driver_abortFlash("unsupported boundary condition on Lower Face")
           ...
           case(NOSLIP_INS)
              ...
              select case(ivar)
              !case (PRES_VAR,DELP_VAR,TVIS_VAR)
              case (PRES_VAR,DELP_VAR,TVIS_VAR,PFUN_VAR,VISC_VAR,DFUN_VAR,CURV_VAR,CPRS_VAR,THCO_VAR,NRMX_VAR,NRMY_VAR,TNLQ_VAR,TNVP_VAR,MDOT_VAR,SMHV_VAR,SMRH_VAR) ! Shizhao
              k = 2*guard+1 
              do i = 1,guard
                 regionData(k-i,1:je,1:ke,ivar)= regionData(i,1:je,1:ke,ivar)
              end do                          
...
!****************************************************************************
           ...
           case(SLIP_INS)
              ...
              select case(ivar)
              !case (PRES_VAR,DELP_VAR,TVIS_VAR)
              case (PRES_VAR,DELP_VAR,TVIS_VAR,DFUN_VAR,PFUN_VAR,CURV_VAR,VISC_VAR,CPRS_VAR,THCO_VAR,NRMX_VAR,NRMY_VAR,TNLQ_VAR,TNVP_VAR,MDOT_VAR,SMHV_VAR,SMRH_VAR)
           ...
           case(OUTFLOW_INS) ! Convective outflow boundary condition.
              ...
              if (isFace) then 
#ifdef TWO_PHASE_SEP
                 if(ivar == VELC_FACE_VAR .OR. ivar == VELI_FACE_VAR ) then ! U velocities on X face grid
#if NDIM == 3
...
!!$                 endif
              ...
              if (ins_predcorrflg) then
              if (isFace) then 
                 if(ivar == VELC_FACE_VAR .OR. ivar == VELI_FACE_VAR ) then ! V velocities on Y face grid
...
!!$                 endif
              ...
              if (ins_predcorrflg) then
              if (isFace) then 
                 if(ivar == VELC_FACE_VAR .OR. ivar == VELI_FACE_VAR ) then ! W velocities on Z face grid
