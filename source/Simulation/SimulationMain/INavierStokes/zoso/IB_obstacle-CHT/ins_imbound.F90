  ...
  do lb = 1,blockCount
        ...
        do j=blkLimits(LOW,JAXIS),blkLimits(HIGH,JAXIS)
         do i=blkLimits(LOW,IAXIS)+1,blkLimits(HIGH,IAXIS)
           ...
           !if(lambdax .gt. 0.0 .and. lambdax .le. 2.0*del(IAXIS)) then
#endif

#ifdef GCELL_FORCING
           if(lambdax .gt. 0.0 .and. lambdax .le. 1.0*del(IAXIS)) then
        ...
        do j=blkLimits(LOW,JAXIS)+1,blkLimits(HIGH,JAXIS)
         do i=blkLimits(LOW,IAXIS),blkLimits(HIGH,IAXIS)
           ...
           !if(lambday .gt. 0.0 .and. lambday .le. 2.0*del(IAXIS)) then
#endif

#ifdef GCELL_FORCING
           if(lambday .gt. 0.0 .and. lambday .le. 1.0*del(IAXIS)) then
