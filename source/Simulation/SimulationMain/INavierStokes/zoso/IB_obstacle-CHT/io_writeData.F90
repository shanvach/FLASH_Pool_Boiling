...
#include "Flash_mpi.h"
  ...
  ! Added by Akash for temporary face var fix
  integer, dimension(9) :: temp_face_arr = (/POLD_FACE_VAR,MGW8_FACE_VAR,MGW9_FACE_VAR,&
                               RH1F_FACE_VAR,RH2F_FACE_VAR,RHDS_FACE_VAR,&
                               VELB_FACE_VAR,VELC_FACE_VAR,VELO_FACE_VAR/)
     ...
     do jproc = 0,io_globalNumProcs - 1
        ...
        if(MASTER_PE /= io_globalMe .AND. jproc == io_globalMe) then
           ...
           if (localNumBlocks > 0) then
              ...
              facext(:,:,:,:) = facevarx(temp_face_arr(i), io_ilo:io_ihi+1, io_jlo:io_jhi, io_klo:io_khi, 1:localNumBlocks)
              if(NDIM .gt. 1) faceyt(:,:,:,:) = facevary(temp_face_arr(i), io_ilo:io_ihi, io_jlo:io_jhi+1, io_klo:io_khi, 1:localNumBlocks)
              if(NDIM .gt. 2) facezt(:,:,:,:) = facevarz(temp_face_arr(i), io_ilo:io_ihi, io_jlo:io_jhi, io_klo:io_khi+1, 1:localNumBlocks)
