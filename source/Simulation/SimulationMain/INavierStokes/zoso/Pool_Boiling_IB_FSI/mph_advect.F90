...
#include "ImBound.h"
  ...
  use Simulation_data, ONLY: sim_nuc_site_x, sim_nuc_site_y, sim_nuc_radii, sim_nuc_site_z, sim_nucSiteDens, sim_ibm_x, sim_ibm_y
     ...
     do k=blkLimitsGC(LOW,KAXIS),blkLimitsGC(HIGH,KAXIS)
      do j=blkLimitsGC(LOW,JAXIS),blkLimitsGC(HIGH,JAXIS)
       do i=blkLimitsGC(LOW,IAXIS),blkLimitsGC(HIGH,IAXIS)
          ...
          solnData(LMDA_VAR,i,j,k) = 0.5 - sqrt((xcell-sim_ibm_x)**2+(ycell-sim_ibm_y)**2)
...
do nuc_index =1,sim_nucSiteDens
  ...
  do lb = 1,blockCount
     ...
     if(abs(ycell+25.0-0.5*del(JAXIS)) .le. tol) then
