...
!#define IB_PRESSURE_JUMP
!#define IB_VEL_TREATMENT
...
#define IB_PRESSURE_JUMP
...
#ifdef IB_VEL_TREATMENT
     do j=blkLimits(LOW,JAXIS),blkLimits(HIGH,JAXIS)
        do i=blkLimits(LOW,IAXIS),blkLimits(HIGH,IAXIS)+1
             if((solnData(LMDA_VAR,i,j,k)+solnData(LMDA_VAR,i-1,j,k))/2.0 .ge. 0.0) then
     ...
     do j=blkLimits(LOW,JAXIS),blkLimits(HIGH,JAXIS)+1
        do i=blkLimits(LOW,IAXIS),blkLimits(HIGH,IAXIS)
             if((solnData(LMDA_VAR,i,j,k)+solnData(LMDA_VAR,i,j-1,k))/2.0 .ge. 0.0) then
