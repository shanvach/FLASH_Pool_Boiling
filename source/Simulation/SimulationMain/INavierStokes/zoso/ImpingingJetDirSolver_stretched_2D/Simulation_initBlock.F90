!!****if* source/Simulation/SimulationMain/INavierStokes/2D/IB_Cyl_parallelIBVP_HYPRE_VD_halfDiamBel/Simulation_initBlock
!!
!! NAME
!!
!!  Simulation_initBlock
!!
!!
!! SYNOPSIS
!!
!!  Simulation_initBlock(integer(in) :: blockID) 
!!                       
!!
!!
!!
!! DESCRIPTION
!!
!!  Initializes fluid data (density, pressure, velocity, etc.) for
!!  a specified block.
!!
!!  Reference:
!!
!! 
!! ARGUMENTS
!!
!!  blockID -          the number of the block to update
!!  myPE   -           my processor number
!!
!! 
!!
!!***

subroutine Simulation_initBlock(blockId)

  use Simulation_data, ONLY : sim_xMin, sim_xMax, &
                              sim_yMin, sim_yMax, &
                              sim_zMin, sim_zMax, &
                              sim_gCell, sim_waveA, &
                              sim_jet_x, sim_jet_z, &
                              sim_nuc_site_x, sim_nuc_site_y,&
                              sim_nuc_site_z, sim_nuc_radii,&
                              sim_nucSiteDens, sim_Tbulk, sim_jet_depth

  use Grid_interface, ONLY : Grid_getDeltas,         &
                             Grid_getBlkIndexLimits, &
                             Grid_getCellCoords,     &
                             Grid_getBlkPtr,         &
                             Grid_releaseBlkPtr,     &
                             Grid_getCellMetrics,    &
                             Grid_getCellCoords,     &
                             Grid_getBlkBoundBox,    &
                             Grid_getBlkCenterCoords

  use Driver_data, ONLY : dr_simTime

  use Grid_data, ONLY : gr_gIndexSize,gr_imin,gr_imax
  implicit none

#include "constants.h"
#include "Flash.h"

  !!$ Arguments -----------------------
  integer, intent(in) :: blockID
  !!$ ---------------------------------
 
  integer :: i, j, k
  integer, dimension(2,MDIM) :: blkLimits, blkLimitsGC
  integer, dimension(MDIM) ::  blIndSize,blIndSizeGC

  real, dimension(MDIM)  :: coord,bsize,del
  real ::  boundBox(2,MDIM)
  real, pointer, dimension(:,:,:,:) :: solnData, facexData,faceyData,facezData

  real :: xcell,xedge,ycell,yedge,zcell
  real, dimension(NXB+1) :: xedge1
  real, dimension(GRID_IHI_GC,3,1) :: dx
  real, dimension(GRID_JHI_GC,3,1) :: dy
  real, dimension(GRID_KHI_GC,3,1) :: dz

  real :: xCoord(GRID_IHI_GC), xFace(GRID_IHI_GC+1), yCoord(GRID_JHI_GC), yFace(GRID_JHI_GC+1)
  real :: A0

  real :: A,B,emp,fs,x0,y0,r0,solnX,z0,x1,y1,z1,x2,y2,z2,d1,d2,d3

  real :: x3,y3,z3,d4,r1,dxc

  real :: fn(10)
  real :: x4,x5,x6,x7,x8
  real :: y4,y5,y6,y7,y8
  real :: d5,d6,d7,d8,d9
  real :: z4,z5,z6,z7,z8 

  real, dimension(17) :: Nuc_radii,Nuc_sites_x,Nuc_sites_z,Nuc_sites_y
  real :: Nuc_dfun
  integer :: Nuc_Index, bli
  real :: th_radii
  real :: xl,xr,yl,yr,dxl,dxr,dyl,dyr

  real :: R_init, jet_vel
  real :: dfun_rect, dlen, distRight, distLeft, dist, xRight, xLeft, yRight
  !----------------------------------------------------------------------
  
  !if (myPE .eq. MASTER_PE) write(*,*) 'InitBlockTime =',dr_simTime

  ! Get nxb, nyb and nxb:
  !call Grid_getBlkIndexSize(blockId,blIndSize,blIndSizeGC)

  !nxb = blIndSize(1)
  !nyb = blIndSize(2)
  !nzb = blIndSize(3)

  ! Get Coord and Bsize for the block:
  ! Bounding box:
  call Grid_getBlkBoundBox(blockId,boundBox)
  bsize(:) = boundBox(2,:) - boundBox(1,:)

  call Grid_getBlkCenterCoords(blockId,coord)

  ! Get blocks dx, dy ,dz:
  !call Grid_getDeltas(blockID,del)

  ! Point to Blocks centered variables:
  call Grid_getBlkPtr(blockID,solnData,CENTER)
  call Grid_getBlkIndexLimits(blockId, blkLimits, blkLimitsGC)

  call Grid_getCellCoords(IAXIS,blockID,CENTER,.true.,xCoord,blkLimitsGC(HIGH,IAXIS))
  call Grid_getCellCoords(JAXIS,blockID,CENTER,.true.,yCoord,blkLimitsGC(HIGH,JAXIS))
  call Grid_getCellCoords(IAXIS,blockID,FACES,.true.,xFace,blkLimitsGC(HIGH,IAXIS)+1)
  call Grid_getCellCoords(JAXIS,blockID,FACES,.true.,yFace,blkLimitsGC(HIGH,JAXIS)+1)
  !do i=1,blkLimits(HIGH,IAXIS)
  !print*,blkLimitsGC(HIGH,IAXIS),blkLimitsGC(HIGH,JAXIS)
  !enddo
  !stop

  ! Point to Blocks face variables: 
  call Grid_getBlkPtr(blockID,facexData,FACEX)
  call Grid_getBlkPtr(blockID,faceyData,FACEY)

#if NDIM == 3
  call Grid_getBlkPtr(blockID,facezData,FACEZ)
#endif

  call Grid_getBlkIndexLimits(blockID,blkLimits,blkLimitsGC,CENTER)
   
  call Grid_getCellMetrics(IAXIS,blockID,LEFT_EDGE, .true.,dx(:,LEFT_EDGE,blockID), blkLimitsGC(HIGH,IAXIS)) 
  call Grid_getCellMetrics(IAXIS,blockID,CENTER,    .true.,dx(:,CENTER,blockID),    blkLimitsGC(HIGH,IAXIS)) 
  call Grid_getCellMetrics(IAXIS,blockID,RIGHT_EDGE,.true.,dx(:,RIGHT_EDGE,blockID),blkLimitsGC(HIGH,IAXIS)) 

  call Grid_getCellMetrics(JAXIS,blockID,LEFT_EDGE, .true.,dy(:,LEFT_EDGE,blockID), blkLimitsGC(HIGH,JAXIS)) 
  call Grid_getCellMetrics(JAXIS,blockID,CENTER,    .true.,dy(:,CENTER,blockID),    blkLimitsGC(HIGH,JAXIS)) 
  call Grid_getCellMetrics(JAXIS,blockID,RIGHT_EDGE,.true.,dy(:,RIGHT_EDGE,blockID),blkLimitsGC(HIGH,JAXIS)) 

#if NDIM ==3
  call Grid_getCellMetrics(KAXIS,blockID,LEFT_EDGE, .true.,dz(:,LEFT_EDGE,blockID), GRID_KHI_GC) 
  call Grid_getCellMetrics(KAXIS,blockID,CENTER,    .true.,dz(:,CENTER,blockID),    GRID_KHI_GC) 
  call Grid_getCellMetrics(KAXIS,blockID,RIGHT_EDGE,.true.,dz(:,RIGHT_EDGE,blockID),GRID_KHI_GC) 
#endif  

#if NDIM == 2
  faceyData(VELC_FACE_VAR,:,:,:) = 1.0/(gr_imax-gr_imin)
#else
  faceyData(VELC_FACE_VAR,:,:,:) = 0.0005
#endif

  sim_jet_x = 0.0
  sim_jet_z = 0.0
  sim_jet_depth = 20
  jet_vel = 1.0
  !- kpd - Initialize the distance function in the 1st quadrant 
  do k=1,blkLimitsGC(HIGH,KAXIS)
     do j=1,blkLimitsGC(HIGH,JAXIS)
        do i=1,blkLimitsGC(HIGH,IAXIS)

           !xcell = coord(IAXIS) - bsize(IAXIS)/2.0 +   &
           !        real(i - NGUARD - 1)*(1/dx(i,CENTER,blockID)) +   &
           !        0.5*(1/dx(i,CENTER,blockID))

           xcell = xCoord(i)
           ycell = yCoord(j)
           !ycell = coord(JAXIS) - bsize(JAXIS)/2.0 +  &
           !        real(j - NGUARD - 1)*(1/dy(j,CENTER,blockID))  +  &
           !        0.5*(1/dy(j,CENTER,blockID))

           zcell = 0.0

           !R_init = 0.2 + 0.3*(-ycell+sim_jet_depth)/sim_jet_depth
           R_init = 0.5

#if NDIM == 3
           zcell = coord(KAXIS) - bsize(KAXIS)/2.0 +  &
                   real(k - NGUARD - 1)/dz(k,CENTER,blockID)  +  &
                   0.5*(1/dz(k,CENTER,blockID))

           !R_init = 0.3 + 0.2*(-ycell+sim_jet_depth)/sim_jet_depth
           R_init = 0.5
#endif        

           solnData(DFUN_VAR,i,j,k)  = min(sqrt((xcell-0.0)**2+(zcell-0.0)**2)-R_init,-ycell+sim_jet_depth)

        enddo
     enddo
  enddo

  do k=1,1
     do j=1,blkLimitsGC(HIGH,JAXIS)+1
        do i=1,blkLimitsGC(HIGH,IAXIS)
  
           xcell = xCoord(i)
           ycell = yFace(j)
           xLeft = xFace(i)
           xRight = xFace(i+1)
          
           zcell = 0.0

           dxc = 1/dx(i,CENTER,blockID)
           dist = sqrt((xcell-sim_jet_x)**2+(zcell-sim_jet_z)**2)
           distLeft = sqrt((xLeft-sim_jet_x)**2+(zcell-sim_jet_z)**2)
           distRight = sqrt((xRight-sim_jet_x)**2+(zcell-sim_jet_z)**2)

           if(ycell .lt. sim_jet_depth) then

             if(distRight .lt. R_init .and. distLeft .gt. R_init) then
                dlen = 0.5-distRight
                faceyData(VELC_FACE_VAR,i,j,k) = jet_vel*(dlen/dxc)
             elseif(distRight .gt. R_init .and. distLeft .lt. R_init) then
                dlen = 0.5-distLeft
                faceyData(VELC_FACE_VAR,i,j,k) = jet_vel*(dlen/dxc)
             elseif(distRight .lt. R_init .and. distLeft .lt. R_init) then
                dlen = dxc
                faceyData(VELC_FACE_VAR,i,j,k) = jet_vel*(dlen/dxc)
             else
                faceyData(VELC_FACE_VAR,i,j,k) = 1.0/(gr_imax-gr_imin)
             endif
           
           endif

        enddo
     enddo
  enddo

  ! For inflow-outflow boundary condition
  do k=1,1
   do i=1,blkLimitsGC(HIGH,IAXIS)
      dxc = 1/dx(i,CENTER,blockID)
      faceyData(VELC_FACE_VAR,i,gr_gIndexSize(JAXIS)+1,k) = 1.0/(gr_gIndexSize(IAXIS)*dxc)
   enddo
  enddo

  ! set values for u,v velocities and pressure
  solnData(PRES_VAR,:,:,:) = 0.0
  solnData(DELP_VAR,:,:,:) = 0.0
  solnData(DUST_VAR,:,:,:) = 0.0
  solnData(TVIS_VAR,:,:,:) = 0.0

  solnData(POOD_VAR,:,:,:) = 0.0
  facexData(SIGO_FACE_VAR,:,:,:) = 0.0
  facexData(SIGOO_FACE_VAR,:,:,:) = 0.0
  faceyData(SIGO_FACE_VAR,:,:,:) = 0.0
  faceyData(SIGOO_FACE_VAR,:,:,:) = 0.0

  solnData(CURV_VAR,:,:,:) = 0.0
  solnData(SIGP_VAR,:,:,:) = 0.0
  solnData(VISC_VAR,:,:,:) = 0.0
  solnData(PFUN_VAR,:,:,:) = 0.0

  facexData(VELC_FACE_VAR,:,:,:) = 0.0
  facexData(RHDS_FACE_VAR,:,:,:) = 0.0
  faceyData(RHDS_FACE_VAR,:,:,:) = 0.0

  facexData(SIGC_FACE_VAR,:,:,:) = 0.0
  faceyData(SIGC_FACE_VAR,:,:,:) = 0.0

  facexData(SIGM_FACE_VAR,:,:,:) = 0.0
  faceyData(SIGM_FACE_VAR,:,:,:) = 0.0
  facexData(RH1F_FACE_VAR,:,:,:) = 0.0
  faceyData(RH1F_FACE_VAR,:,:,:) = 0.0
  facexData(RH2F_FACE_VAR,:,:,:) = 0.0
  faceyData(RH2F_FACE_VAR,:,:,:) = 0.0

#if NDIM == 3
  facezData(VELC_FACE_VAR,:,:,:) = 0.0
  facezData(RHDS_FACE_VAR,:,:,:) = 0.0
  facezData(SIGM_FACE_VAR,:,:,:) = 0.0
  facezData(RH1F_FACE_VAR,:,:,:) = 0.0
  facezData(RH2F_FACE_VAR,:,:,:) = 0.0
  facezData(SIGC_FACE_VAR,:,:,:) = 0.0
  facezData(SIGOO_FACE_VAR,:,:,:) = 0.0
  facezData(SIGO_FACE_VAR,:,:,:) = 0.0
#endif

  ! Release pointer
  call Grid_releaseBlkPtr(blockID,solnData,CENTER)

  call Grid_releaseBlkPtr(blockID,facexData,FACEX)
  call Grid_releaseBlkPtr(blockID,faceyData,FACEY)

#if NDIM == 3
  call Grid_releaseBlkPtr(blockID,facezData,FACEZ)
#endif

  return

111    format (i4,3x,i4)
112    format (3(3x,e12.4))

end subroutine Simulation_initBlock
