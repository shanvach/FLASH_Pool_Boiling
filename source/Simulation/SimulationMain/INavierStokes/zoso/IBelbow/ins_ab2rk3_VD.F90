...
!***********************************************************************************************
  ...
  if(ins_meshMe .eq. MASTER_PE) write(*,*) 'initial Qin',ins_Qin
...
!*************************************************************************************************
  ...
  if (ins_meshMe .eq. MASTER_PE)  write(*,*) 'Total IB Time =',ETIB
     ...
     !   call outtotecplot(dr_globalMe,dr_simtime,dr_dt,dr_nstep,dr_nstep, &
     !                     0.0,listOfBlocks,count,firstfileflag)
     !endif 
  ...
  do lb = 1,blockCount
    ...
    !open(1, file = 'divu3.txt', action = 'write', access = 'append') !status='replace'
    !    do j=blkLimits(LOW,JAXIS),blkLimits(HIGH,JAXIS)
    !       do i=blkLimits(LOW,IAXIS),blkLimits(HIGH,IAXIS)
    !         Mdens = (rho1x(i,j,k) + rho2x(i,j,k))
    !         write(1,*) i,j,facexData(VELC_FACE_VAR,i,j,1),ins_meshMe
    !          write(1,*)
    !          i,j,facexData(VELC_FACE_VAR,i,j,1),faceyData(VELC_FACE_VAR,i,j,1)
...
!***********************************************************************************************
  ...
  do lb = 1,blockCount
     ...
     call ins_corrector_VD_IB( facexData(VELC_FACE_VAR,:,:,:),&
                         faceyData(VELC_FACE_VAR,:,:,:),&
                         facezData(VELC_FACE_VAR,:,:,:),&
