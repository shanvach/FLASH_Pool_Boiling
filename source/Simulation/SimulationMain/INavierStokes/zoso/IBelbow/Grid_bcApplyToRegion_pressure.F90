           ...
           case(DIRICHLET)
              ...
              if (isFace) then ! Set to zero velocities normal to boundary, up to boundary
                ! if(ivar == VELC_FACE_VAR) then
                 !k = guard+1
                 !regionData(guard+1,1:je,1:ke,ivar)= regionData(guard+2,1:je,1:ke,ivar)
                 !do i = 1,guard
           ...
           case(INFLOW_INS)
              ...
              if (isFace) then ! Set to zero velocities normal to boundary, up to boundary
                 ...
                 do i = 1,guard+1
                    ...
                    regionData(i,1:je,1:ke,ivar)= 0.001
                 end do
                 else
                 k = 2*guard+2
                 do i = 1,guard
...
!!              call Driver_abortFlash("unsupported boundary condition on Lower Face")
           ...
           case(DIRICHLET)
              ...
              ! First Order Upwind - staggered in the face:
              !regionData(guard+1,1:je,1:ke,ivar)= regionData(guard,1:je,1:ke,ivar) 
              !do i =1,guard
              !   regionData(k+i,1:je,1:ke,ivar) = regionData(k+i-1,1:je,1:ke,ivar)
              !enddo
              k = k+1 
