...
#include "Flash.h"
  ...
  use Simulation_data, only: sim_vel, sim_vel_flg, sim_pres_flg,&
                       sim_vel_top, sim_vel_flg_top, sim_pres_flg_top, sim_dfun_top
       ...
       if(face == LOW) then ! Level 3
         ...
         if(axis == IAXIS) then ! Level 3a
             ...
             if(gridDataStruct == CENTER) then
               ...
               !k = 2*guard+1
               !do i = 1,guard
               !   !regionData(i,1:je,1:ke,ivar) = 2*ht_Twall_low*(1-mph_bcFlag(:,:,blockHandle)) &
               !   !                             + 2*ht_Tsat*(mph_bcFlag(:,:,blockHandle)) & 
               ...
               k = 2*guard+1
               do i = 1,guard
                  regionData(i,1:je,1:ke,ivar) = sim_pres_flg(ja:jb,ka:kb,blockHandle)*regionData(k-i,1:je,1:ke,ivar)
               end do
               ...
               if(ivar == VELC_FACE_VAR .or. ivar == VELI_FACE_VAR) then
                        ...
                        regionData(guard+1,1:je,1:ke,ivar)= sim_vel(ja:jb,ka:kb,blockHandle)*&
                                                         (1-sim_vel_flg(ja:jb,ka:kb,blockHandle)) + &
                                                            sim_vel_flg(ja:jb,ka:kb,blockHandle)*&
                                                            regionData(guard+1,1:je,1:ke,ivar)
                        ...
                        k = 2*guard+1
                        do i = 1,guard
                        regionData(i,1:je,1:ke,ivar)=-sim_pres_flg(ja:jb,ka:kb,blockHandle)*regionData(guard+1,1:je,1:ke,ivar)
                        end do
                        endif
