...
subroutine gr_hypreCreateMatrix_KPD(iVar, bcTypes, bcValues, dt, &
  ...
  use Simulation_data, only: sim_pres_flg, sim_pres_flg_top
        ...
        do j = blkLimits(LOW, JAXIS), blkLimits(HIGH, JAXIS)
           do k = blkLimits(LOW, KAXIS), blkLimits(HIGH, KAXIS)                    
              ...
              if ((j /= blkLimits(LOW, JAXIS)) .or. (faces(1,JAXIS) == NOT_BOUNDARY)) then                  
                 ...
                 MdensYL = (1 - sim_pres_flg(i,k,blockID))*0.5*&
                           2*(facevary(RH1F_FACE_VAR,i ,j,k)+facevary(RH2F_FACE_VAR,i  ,j  ,k))
              ...
              if ((j /= blkLimits(HIGH, JAXIS)) .or. (faces(2,JAXIS) == NOT_BOUNDARY)) then                  
                 ...
                 !MdensYR = (1 - sim_pres_flg_top(i,k,blockID))*0.5*&
                 !          2*(facevary(RH1F_FACE_VAR,i,j+1,k)+facevary(RH2F_FACE_VAR,i  ,j+1,k))
...
!    end if
     ...
     do i = IAXIS, NDIM      !- kpd - Go through x-dir, y-dir, & z-dir
        do j = LOW, HIGH     !        Go through 1,2
           ...
           !        bcValues(:,dir), dt, theta, del(i), gr_hypreLower(lb,:), dirichlet_multiplier, &
           !        faceAreas(i,:,:,:), solnVec)
           !end if
