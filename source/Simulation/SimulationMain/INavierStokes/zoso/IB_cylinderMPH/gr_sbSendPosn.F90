...
subroutine gr_sbSendPosn()
  ...
  use Imbound_data, only: ib_dfun_flg, ib_vel_flg
  implicit none
  call Timers_start("send_particles_posn")
  if(ib_vel_flg) call gr_sbStoreParticlesPerProc()
  if(ib_vel_flg) call gr_sbSendParticleCount()
  if(ib_vel_flg) call gr_sbSendParticles()
