...
!              write(333,'(3I8,g24.10)') blockID,i,j,facexData(FORC_FACE_VAR,i,j,1)
     ...
     do j=1,NYB 
        ...
        yj = coord(JAXIS) - bsize(JAXIS)/2.0 + real(j-1)*dy + dy/2.0
     ...
     ! Write unum:
     arraylb(:,:,1) =    &
     sngl(facevarxx(NGUARD+1:nxc,NGUARD+1:nyc-1))
     i = TecDat(ijk,arraylb,0)
...
!  close(333)
  ...
  do lb = 1,blockcount
     ...
     ! Write vnum:
     arraylb2(:,:,1) = & 
     sngl(facevaryy(NGUARD+1:nxc-1,NGUARD+1:nyc))
     i = TecDat(ijk2,arraylb2,0)
  ...
  do lb = 1,blockcount
     ...
     do j=NGUARD+1,nyc-1              
        do i=NGUARD+1,nxc-1
            ...
            presvar(i,j) = solnData(TEMP_VAR,i,j,1)
