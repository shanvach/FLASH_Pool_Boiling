...
!  call int2char(mype,index_mype)
  ...
  do lb = 1,blockcount
     ...
     facevarxx2 = facexData(VELI_FACE_VAR,:,:,1)
     facevaryy2 = faceyData(VELI_FACE_VAR,:,:,1)
     ...
     ! U velocity: u(nxb+1,nyb+1)
     ! --------------------------
     tpuint = 0.5*(facevarxx2(NGUARD+1:nxc,NGUARD:nyc-1)+  &
                facevarxx2(NGUARD+1:nxc,NGUARD+1:nyc) )


     ! V velocity: v(nxb+1,nyb+1)
     ! --------------------------                           
     tpvint = 0.5*(facevaryy2(NGUARD:nxc-1,NGUARD+1:nyc) + &
                facevaryy2(NGUARD+1:nxc,NGUARD+1:nyc) )
     ...
     call centervals2corners(NGUARD,NXB,NYB,nxc,nyc, &
                            solnData(NRMX_VAR,:,:,1),tnx) 

     call centervals2corners(NGUARD,NXB,NYB,nxc,nyc, &
                            solnData(NRMY_VAR,:,:,1),tny) 
...
!      goto 100
      ...
      arraylb(:,:,1) = sngl(tpuint)
      i = TecDat(ijk,arraylb,0)

      arraylb(:,:,1) = sngl(tpvint)
      i = TecDat(ijk,arraylb,0)
